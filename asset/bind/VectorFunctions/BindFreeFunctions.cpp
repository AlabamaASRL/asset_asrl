#include <bind/VectorFunctions/BindFreeFunctions.h>
...
void ASSET::BindFreeFunctions(FunctionRegistry& reg, py::module& m) {
  using Gen = GenericFunction<-1, -1>;
  using GenS = GenericFunction<-1, 1>;
  ...
  */
  ...
  m.def("arctan2",
        [](const GenS& y, const GenS& x) { return GenS(ArcTan2Op().eval(StackedOutputs {y, x})); });
  m.def("arctan2",
        [](const ELEM& y, const ELEM& x) { return GenS(ArcTan2Op().eval(StackedOutputs {y, x})); });
  m.def("arctan2",
        [](const ELEM& y, const GenS& x) { return GenS(ArcTan2Op().eval(StackedOutputs {y, x})); });
  m.def("arctan2",
        [](const GenS& y, const ELEM& x) { return GenS(ArcTan2Op().eval(StackedOutputs {y, x})); });

  m.def("divtest",
        [](const Gen& a, const GenS& b) { return Gen(VectorScalarFunctionDivision<Gen, GenS> {a, b}); });
  m.def("divtest",
        [](const Gen& a, const ELEM& b) { return Gen(VectorScalarFunctionDivision<Gen, ELEM> {a, b}); });
}
