...
namespace ASSET {
	...
	void LambertSolverBuildMultiRev(FunctionRegistry& reg, py::module& m);
	...
	void lambert_izzo_impl(const Scalar* r1_in, const Scalar* r2_in, Scalar t, const Scalar& mu, //INPUT
		bool lw, const Scalar& N, bool right, //INPUT
		Scalar* v1, Scalar* v2, Scalar& a, Scalar& p, Scalar& theta, int& iter)//OUTPUT
	{
		...
		auto x2tof = [](Scalar x, Scalar s, Scalar c, const int & lw, Scalar N) {
			...
			if (a > 0)
			{
				return (a * sqrt(a) * ((alfa - sin(alfa)) - (beta - sin(beta)) + 2 * atan(1) * 4 * N));
			}
			else
			{
        ...
        // Regula-falsi iterations
            ...
            err = 1;
            i_count = 0;
            y1 = log(x2tof((exp(x1 * D_ALP) - 1.0) / (exp(x1 * D_ALP) + 1.0), s, c, lw, N)) - log(t);
            y2 = log(x2tof((exp(x2 * D_ALP) - 1.0) / (exp(x2 * D_ALP) + 1.0), s, c, lw, N)) - log(t);
        		while ((err > tolerance) && (y1 != y2))
        		{
        		    i_count++;
                    x_new = (x1 * y2 - y1 * x2) / (y2 - y1);
        		    y_new = log(x2tof((exp(x_new * D_ALP) - 1.0) / (exp(x_new * D_ALP) + 1.0), s, c, lw, N)) - log(t);
        			x1 = x2;
        			y1 = y2;
        			x2 = x_new;
        			y2 = y_new;
        		...
        		iter = i_count;
        		x = (exp(x_new * D_ALP) - 1.0) / (exp(x_new * D_ALP) + 1.0);
	...
	template<class Scalar>
	std::array<Vector3<Scalar>, 2> lambert_izzo_multirev(const Vector3<Scalar>& R1t, const Vector3<Scalar>& R2t, Scalar tf, Scalar mu, bool lw, Scalar N, bool right) {
		...
		lambert_izzo_impl(R1.data(), R2.data(), tf, mu, lw, N, right,
