...
void ASSET::LambertSolversBuild(FunctionRegistry& reg, py::module& m) {
	...
	m.def("lambert_izzo",
		[](const Vector3<double>& R1, const Vector3<double>& R2, double dt, double mu, bool longway,int Nrevs, bool rightbranch) {
	...
	m.def("lambert_izzo",
		[](ConstEigenRef<NumpyMat> R1s, ConstEigenRef<NumpyMat> R2s, ConstEigenRef<VectorX<double>> dts, double mu, const std::vector<bool> & longways,
			EigenRef<NumpyMat> V1s, EigenRef<NumpyMat> V2s,
			int axis,bool vectorize) {
			...
			for (int i = 0; i < Packs; i++) {
				...
				lambert_izzo_impl(R1ss, R2ss, dtss, mu,
					lwss,
					0, false,
					V1ss, V2ss,
					ecodess);
