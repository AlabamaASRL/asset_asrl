...
struct RKStepper_Impl {
  ...
  static auto ComputeXf(const DODE& ode, const Ks& ks) {
    ...
    auto xi =
        KthStageSum2<Stg, 0, decltype(args), decltype(empty)>(ode, args, empty);
  ...
  static auto make_state(const DODE& ode, const Xtype& x0, const Titype& ti,
                         const Utype& u, const Ptype& p) {
  ...
  static auto KthStageSum2(const DODE& ode, const Args& args, const Ks& ks) {
    if constexpr (Elem == Stg + 1) {
      ...
      if constexpr (RKCoeffs<RKOp>::ACoeffs[Stg][Elem] == 0.0) {
        ...
        return KthStageSum2<Stg, Elem + 1, Args, decltype(knew)>(ode, args,
                                                                 knew);
      }
    };
  }
  ...
  static auto FinalStateSum(const DODE& ode, const Args& args, const KF& kf) {
    if constexpr (RKOp == RK4Classic) {
      return make_sum(
          kf * BCoeff<3>().value, args.template head<DODE::XV>(),
      ...
      return make_sum(
          kf * BCoeff<5>(), args.template head<DODE::XV>(),
          args.template tail<DODE::XV*(RKCoeffs<RKOp>::Stages - 1)>()
                  .template segment<DODE::XV, DODE::XV * 0>() *
              BCoeff<0>(),
          args.template tail<DODE::XV*(RKCoeffs<RKOp>::Stages - 1)>()
...
struct RKStepper_Impl_NEW {
    ...
    static auto ComputeXf(const DODE& ode, const Ks& ks) {
        ...
        auto tlam = [&]() {
            if constexpr (Stg == -1)return t0;
            else return t0 + h * RKCoeffs<RKOp>::Times[Stg];
        };
    ...
    static auto KthStageSum(const DODE& ode, const Args& args, const Ks& ks) {
        if constexpr (Elem == Stg + 1) {
            ...
            if constexpr (RKCoeffs<RKOp>::ACoeffs[Stg][Elem] == 0.0) {
                ...
                auto next = args.template tail<SZ_PROD<(Stg + 1), DODE::XV>::value>((Stg + 1)*ode.XVars())
                    .template segment<DODE::XV, SZ_PROD<Elem, DODE::XV>::value>(ode.XVars()*Elem,ode.XVars()) *
                    ACoeff<Stg, Elem>().value;
                auto knew = std::tuple_cat(ks, std::make_tuple(next));
                return KthStageSum<Stg, Elem + 1, Args, decltype(knew)>(ode, args,
                    knew);
            }
        };
    }
...
template <class DODE, RKOptions RKOp>
struct RKStepper : VectorExpression<RKStepper<DODE, RKOp>,
                                    RKStepper_Impl<DODE, RKOp>, const DODE&> {
  using Base = VectorExpression<RKStepper<DODE, RKOp>,
                                RKStepper_Impl<DODE, RKOp>, const DODE&>;
  ///using Base::Base;
  
  RKStepper(const DODE & ode):Base(ode){}
};


template <class DODE, RKOptions RKOp>
struct RKStepper_NEW : VectorExpression<RKStepper_NEW<DODE, RKOp>,
    RKStepper_Impl_NEW<DODE, RKOp>, const DODE&> {
    using Base = VectorExpression<RKStepper_NEW<DODE, RKOp>,
        RKStepper_Impl_NEW<DODE, RKOp>, const DODE&>;
    // using Base::Base;
...
template <class DODE, RKOptions RKOp>
struct RKStepper2
    : VectorFunction<RKStepper2<DODE, RKOp>, SZ_SUM<DODE::IRC, 1>::value,
                     DODE::IRC, FDiffFwd, FDiffFwd> {
  using Base =
      VectorFunction<RKStepper2<DODE, RKOp>, SZ_SUM<DODE::IRC, 1>::value,
                     DODE::IRC, FDiffFwd, FDiffFwd>;
  ...
  inline void compute_impl(const Eigen::MatrixBase<InType>& x,
    ...
    for (auto& K : Kvals) K = ODEDeriv<Scalar>::Zero(this->ode.ORows());
  ...
  inline void compute_jacobian_impl1(
    ...
    Jacobian<Scalar> Xijac =
        Jacobian<Scalar>::Zero(this->ORows(), this->IRows());
    ;
    ...
    KXjacs[0]
        .col(this->ode.TVar())
        .template head<DODE::XV>(this->ode.XVars()) -= Kvals[0];
    KXjacs[0]
        .col(this->IRows() - 1)
        .template head<DODE::XV>(this->ode.XVars()) += Kvals[0];
    Kvals[0] *= h;
    ...
    for (int i = 0; i < Stgsm1; i++) {
      ...
      const int js = isDiag ? i : 0;
      for (int j = js; j < ip1; j++) {
        xtup.template head<DODE::XV>(this->ode.XVars()) +=
            RKData::ACoeffs[i][j] * Kvals[j];
