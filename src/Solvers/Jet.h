...
namespace ASSET {
    ...
    struct Jet {
        ...
        static void print_progress(int i, double tsec,int NumJobs,int NumConv, int NumAcc, int NumNoConv, int NumDiv) {
            ...
            fmt::print(" Remaining Time : "); sminhrs(remtime);
            fmt::print(" Elapsed Time   : "); sminhrs(tsec);
            fmt::print(" Progress       : "); fmt::print(cyan, "{0:>10.2f} %  \n", prog);
            fmt::print(" ["); fmt::print(green, "{0:#^{1}}{0:.^{2}}", "", len, wspace); fmt::print("]");
            fmt::print("\n\n");
            fmt::print("  Completed        : "); fmt::print(cyan, "{0:>10}/{1:<10}   \n", (i + 1), NumJobs);

            fmt::print("    Optimal        : "); fmt::print(cyan, "{0:>10}/{1:<10}   \n", NumConv, NumJobs);
            fmt::print("    Acceptable     : "); fmt::print(cyan, "{0:>10}/{1:<10}   \n", NumAcc, NumJobs);
            fmt::print("    Not Converged  : "); fmt::print(cyan, "{0:>10}/{1:<10}   \n", NumNoConv, NumJobs);
            fmt::print("    Diverged       : "); fmt::print(cyan, "{0:>10}/{1:<10}   \n", NumDiv, NumJobs);

            if (i < (NumJobs - 1))fmt::print("\033[11F");
            else fmt::print("\n");
        };
        ...
        static std::vector<std::shared_ptr<T>> map( 
            ...
            for (int i = 0; i < NumJobs; i++) {
                ...
                if (verbose) {
                    if (flag == PSIOPT::ConvergenceFlags::CONVERGED)   NumConv++;
                    if (flag == PSIOPT::ConvergenceFlags::ACCEPTABLE)  NumAcc++;
                    if (flag == PSIOPT::ConvergenceFlags::NOTCONVERGED)NumNoConv++;
                    if (flag == PSIOPT::ConvergenceFlags::DIVERGING)   NumDiv++;
        ...
        template<class T, class Args1,class Args2>
        static std::vector<std::shared_ptr<T>> map(
            std::function<std::shared_ptr<T>(Args1)> genfunc,
            const std::vector<Args2>& args,
            int nt,
            bool verbose) {
        ...
        template<class T>
        static std::vector<std::shared_ptr<T>> map(
            const std::vector<std::shared_ptr<T>>& optprobs,
            int nt,
            bool verbose) {
        ...
        static void Build(py::module& m) {
            ...
            obj.def_static("map",
                [](std::function<std::shared_ptr<OptimizationProblemBase>(py::detail::args_proxy)> genfun, const std::vector<py::args>& args, int nt) {
                    return Jet::map(genfun,args, nt, true);
                }, py::call_guard<py::gil_scoped_release>());
            ...
            obj.def_static("map",
                [](std::function<std::shared_ptr<OptimizationProblemBase>(py::detail::args_proxy)> genfun, const std::vector<py::args>& args, int nt, bool v) {
                    return Jet::map(genfun, args, nt, v);
                }, py::call_guard<py::gil_scoped_release>());
