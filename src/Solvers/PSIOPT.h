...
struct PSIOPT {
  ...
  static QPOrderingModes strto_OrderingMode(const std::string& str) {
      ...
      if (str.compare("MINDEG") == 0) return MINDEG;
      else if (str.compare("METIS") == 0) return METIS;
      else if (str.compare("PARMETIS") == 0) return PARMETIS;
      else {
          auto msg = fmt::format("Unrecognized QPOrderingMode: {0}\n"
              "Valid Options Are: MINDEG , METIS, PARMETIS ", str);
  ...
  static LineSearchModes strto_LineSearchMode(const std::string& str) {
      ...
      if (str.compare("L1") == 0) return L1;
      else if (str.compare("NOLS") == 0) return NOLS;
      else if (str.compare("LANG") == 0) return LANG;
      else if (str.compare("AUGLANG") == 0) return AUGLANG;
      else {
          auto msg = fmt::format("Unrecognized LineSearchMode: {0}\n"
              "Valid Options Are: AUGLANG, LANG, L1, NOLS ", str);
          throw std::invalid_argument(msg);
          return L1;
      }
  }
  static BarrierModes    strto_BarrierMode(const std::string& str) {
      ...
      if (str.compare("PROBE") == 0) return PROBE;
      else if (str.compare("LOQO") == 0) return LOQO;
      else {
          auto msg = fmt::format("Unrecognized BarrierMode: {0}\n"
              "Valid Options Are: LOQO, PROBE ", str);
          throw std::invalid_argument(msg);
          return LOQO;
      }
  }
  ...
  using EarlyCallBackType = std::function<int(
      int, double, EigenRef<VectorXd>, double, EigenRef<VectorXd>,
      EigenRef<VectorXd>, Eigen::SparseMatrix<double, Eigen::RowMajor>&)>;

  using LateCallBackType = std::function<int(
      const IterateInfo&, ConstEigenRef<VectorXd>, ConstEigenRef<VectorXd>)>;

  EarlyCallBackType
      EarlyCallBack;  // = [](int i, EigenRef<VectorXd> XSL, EigenRef<VectorXd>
                      // GX, EigenRef<VectorXd> AGXFX) {return 0; };
  bool EarlyCallBackEnabled = false;
  LateCallBackType
      LateCallBack;  // = [](const IterateInfo& i, EigenRef<VectorXd> XSL,
  ...
  void setQPParams() {
    ...
    if (this->CNRMode) this->KKTSol.m_threads = this->QPThreads;
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////
  void apply_reset_slacks(Eigen::Ref<Eigen::VectorXd> S,
  ...
  void barrier_gradient(Eigen::Ref<Eigen::VectorXd> LI,
                        Eigen::Ref<Eigen::VectorXd> AGS) const {
    AGS = LI;
  }
  ...
  double LOQOMu(Eigen::Ref<Eigen::VectorXd> S, Eigen::Ref<Eigen::VectorXd> LI,
  ...
  double MPCMu(Eigen::Ref<Eigen::VectorXd> S, Eigen::Ref<Eigen::VectorXd> LI,
               double avgcomp, double mincomp) const {
    double navgcomp = 0;
    double nmincomp = 0;
    double nmaxcomp = 0;
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
  void evalKKT(double ObjScale, ConstEigenRef<VectorXd> XSL, double& val,
               EigenRef<VectorXd> GX, EigenRef<VectorXd> AGXS_FX,
               Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat) {
    this->nlp->evalKKT(
        ObjScale, XSL.head(this->PrimalVars),
  ...
  void evalKKTNO(double ObjScale, ConstEigenRef<VectorXd> XSL, double& val,
                 EigenRef<VectorXd> GX, EigenRef<VectorXd> AGXS_FX,
                 Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat) {
    this->nlp->evalKKTNO(
        ObjScale, XSL.head(this->PrimalVars),
  ...
  void evalAUG(double ObjScale, ConstEigenRef<VectorXd> XSL, double& val,
               EigenRef<VectorXd> GX, EigenRef<VectorXd> AGXS_FX,
               Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat) {
    this->nlp->evalAUG(
        ObjScale, XSL.head(this->PrimalVars),
  ...
  void evalSOE(double ObjScale, ConstEigenRef<VectorXd> XSL, double& val,
               EigenRef<VectorXd> GX, EigenRef<VectorXd> AGXS_FX,
               Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat) {
    this->nlp->evalSOE(
        ObjScale, XSL.head(this->PrimalVars),
  ...
  void evalRHS(double ObjScale, const Eigen::Ref<const Eigen::VectorXd>& XSL,
  ...
  void fill_iter_info(Eigen::Ref<Eigen::VectorXd> XSL,
                      Eigen::Ref<Eigen::VectorXd> RHS, double pobj, double bobj,
                      double mu, IterateInfo& iter) const;

  void evalNLP(int algmode, double ObjScale, ConstEigenRef<VectorXd> XSL,
  ...
  void print_ExitStats(ConvergenceFlags ExitCode,const std::vector<IterateInfo>& iters,
      double tottime, double nlptime, double qptime);

  fmt::text_style calculate_color(double val, double targ, double acc);

  int factor_impl(bool docompute, bool ZFac, double ipurt, double incpurt0,
  ...
  Eigen::VectorXd alg_impl(AlgorithmModes algmode, BarrierModes barmode,
                           LineSearchModes lsmode, double ObjScale, double MuI,
                           Eigen::Ref<Eigen::VectorXd> xsl);


  Eigen::VectorXd init_impl(const Eigen::VectorXd& x, double Mu,
                            bool docompute);


  double ls_impl(LineSearchModes lsmode, double ObjScale, double Mu, 
