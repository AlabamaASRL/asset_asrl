...
void ASSET::PSIOPT::setNLP(std::shared_ptr<NonLinearProgram> np) {
  ...
  if (storespmat) spmat = this->KKTSol.getMatrix();
  this->QPanalyzed = false;
}

void ASSET::PSIOPT::max_primal_dual_step(Eigen::Ref<Eigen::VectorXd> XSL,
                                         Eigen::Ref<Eigen::VectorXd> DXSL,
                                         double bfrac, double& alphap,
                                         double& alphad) {
  double Smax = this->max_step_to_boundary(this->getSlacks(XSL),
                                           this->getSlacks(DXSL), bfrac);
  double Lmax = this->max_step_to_boundary(this->getIqLmults(XSL),
                                           this->getIqLmults(DXSL), bfrac);
  ...
  if (InequalCons > 0) this->getSlacks(DXSL) *= slackstep;
  if (EqualCons > 0) this->getEqLmults(DXSL) *= eqmultstep;
  if (InequalCons > 0) this->getIqLmults(DXSL) *= iqmultstep;
...
void ASSET::PSIOPT::fill_iter_info(Eigen::Ref<Eigen::VectorXd> XSL,
  ...
  if (InequalCons > 0) {
    ...
    this->complementarity(this->getSlacks(XSL), this->getIqLmults(XSL), avgcomp,
                          mincomp, maxcomp);
...
void ASSET::PSIOPT::evalNLP(
    int algmode, double ObjScale, ConstEigenRef<VectorXd> XSL, double& val,
...
ASSET::PSIOPT::ConvergenceFlags ASSET::PSIOPT::convergeCheck(
...
void ASSET::PSIOPT::print_stats() {
  ...
  fmt::print(" Primal Variables         : "); fmt::print(cyan,"{:<10}\n", this->PrimalVars);
  fmt::print(" Equality Constraints     : "); fmt::print(cyan, "{:<10}\n", this->EqualCons);
  fmt::print(" Inequality Constraints   : "); fmt::print(cyan, "{:<10}\n", this->InequalCons);
  fmt::print("\n");
  fmt::print(" KKT-Matrix DIM (P+E+2*I) : "); fmt::print(cyan, "{:<10}\n", this->KKTdim);
  fmt::print(" KKT-Matrix NNZs          : "); fmt::print(cyan, "{:<10}\n", this->KKTSol.getMatrix().nonZeros());
  fmt::print(" KKT-Matrix NNZ%          : "); fmt::print(cyan, "{:.6f}%\n", 100.0 * double(this->KKTSol.getMatrix().nonZeros()) /
...
void ASSET::PSIOPT::print_last_iterate(const std::vector<IterateInfo>& iters)
{
    ...
    if (last.iter % 10 == 0) {
        if (WideConsole) {
            fmt::print("{0:=^{1}}\n", "", 159);
            fmt::print("|Iter| Mu Val | Prim Obj |  Bar Obj |  KKT Inf |  Bar Inf | ECons Inf| ICons Inf|Max EMult|Max IMult| AlphaP | AlphaD | AlphaT | Merit Val|LSI|PPS|HFI| HPert |\n");
        }
        else {
            fmt::print("{0:=^{1}}\n", "", 119);
            fmt::print("|Iter| Mu Val | Prim Obj |  Bar Obj |  KKT Inf |  Bar Inf | ECons Inf| ICons Inf| AlphaP | AlphaD |LS| PPS |HF| HPert |\n");
        }
    ...
    if (iters.size() > 1) {
        ...
        PHashcol =
            (iters.back().PrimObj <= iters[iters.size() - 2].PrimObj)
            ? GCol
            : BCol;
        BOHashcol =
            (iters.back().BarrObj <= iters[iters.size() - 2].BarrObj)
            ? GCol
            : BCol;
        BHashcol =
            (iters.back().BarrInf <= iters[iters.size() - 2].BarrInf)
            ? GCol
            : BCol;
        EHashcol =
            (iters.back().EConInf <= iters[iters.size() - 2].EConInf)
            ? GCol
            : BCol;
        IHashcol =
            (iters.back().IConInf <= iters[iters.size() - 2].IConInf)
            ? GCol
            : BCol;
        KHashcol =
            (iters.back().KKTInf <= iters[iters.size() - 2].KKTInf)
            ? GCol
            : BCol;
    }
    ...
    if (WideConsole) {
        fmt::print("{:>9.3e}|{:>9.3e}|{:>8.2e}|{:>8.2e}|{:>8.2e}|{:>10.3e}|{:>3}|{:>3}|{:>3}|{:>6.1e}|\n",
            last.MaxEMult, last.MaxIMult, last.alphaP, last.alphaD, last.alphaT,
            last.MeritVal, last.LSiters, last.PPivots, last.Hfacs, last.Hpert);
    }
    else {
        fmt::print("{:>8.2e}|{:>8.2e}|{:>2}|{:>5}|{:>2}|{:>6.1e}|\n",
             last.alphaT*last.alphaP, last.alphaT * last.alphaD, last.LSiters, last.PPivots, last.Hfacs, last.Hpert);
    }
...
void ASSET::PSIOPT::print_ExitStats(ConvergenceFlags ExitCode,const std::vector<IterateInfo>& iters, double tottime, double nlptime,  double qptime)
{
    auto last = iters.back();
    fmt::text_style Kcol = calculate_color(last.KKTInf, this->KKTtol, this->AccKKTtol);
    fmt::text_style Bcol = calculate_color(last.BarrInf, this->Bartol, this->AccBartol);
    ...
    auto Printtime = [&](const char* msg, double t1) {
        fmt::print(msg); fmt::print(TColor, "{0:>10.3f} ms {1:>10.3f} ms/iter\n", t1, double(t1 / iternum));
    };
    ...
    if (this->PrintLevel < 2) {
        ...
        fmt::print(" Iterations : "); fmt::print("{:<5}\n", iternum);
        fmt::print(" Prim Obj   : "); fmt::print("{:<15.8e}\n", last.PrimObj);
        fmt::print(" KKT Inf    : "); fmt::print(Kcol, "{:<15.8e}\n", last.KKTInf);
        fmt::print(" Bar Inf    : "); fmt::print(Bcol, "{:<15.8e}\n", last.BarrInf);
        fmt::print(" ECons Inf  : "); fmt::print(Ecol, "{:<15.8e}\n", last.EConInf);
        fmt::print(" ICons Inf  : "); fmt::print(Icol, "{:<15.8e}\n", last.IConInf);
...
int ASSET::PSIOPT::factor_impl(bool docompute, bool Zfac, double ipurt,
                               double incpurt0, double incpurt,
                               double& finalpert) {
  auto Inertia = [&]() {
    return this->KKTSol.neigs() - (this->EqualCons + this->InequalCons);
  };
  auto RankDef = [&]() {
  ...
  auto Perturb = [&](double p) {
    this->nlp->perturbKKTPDiags(p, this->KKTSol.getMatrix());
  };
...
Eigen::VectorXd ASSET::PSIOPT::alg_impl(AlgorithmModes algmode,
  ...
  for (int i = 0; i < this->MaxIters; i++) {
    ...
    if (this->InequalCons > 0) {
      ...
      this->barrier_hessian(this->KKTSol.getMatrix(), this->getSlacks(XSL),
                            this->getIqLmults(XSL), Mu);
      this->complementarity(this->getSlacks(XSL), this->getIqLmults(XSL),
    ...
    if (this->EarlyCallBackEnabled) {
      CBtimer.start();
      this->EarlyCallBack(i, ObjScale, XSL, PrimObj, PGX, RHS,
                          this->KKTSol.getMatrix());
      CBtimer.stop();
    }
    QPtimer.start();
    ...
    if (this->InequalCons > 0) {
      switch (barmode) {
        case BarrierModes::PROBE:
          this->barrier_gradient(this->getIqLmults(XSL),
                                 this->getDualGrad(RHS));
          DXSL = -this->KKTSol.solve(RHS);
          this->max_primal_dual_step(XSL, DXSL, this->BoundFraction, alphap,
                                     alphad);
          Temp = XSL + DXSL;
          Mu = this->MPCMu(this->getSlacks(Temp), this->getIqLmults(Temp),
                           avgcomp, mincomp);
          
          break;
        case BarrierModes::LOQO:
          Mu = this->LOQOMu(this->getSlacks(XSL), this->getIqLmults(XSL),
                            avgcomp, mincomp);
          break;
      ...
      this->barrier_gradient(this->getSlacks(XSL), this->getIqLmults(XSL), Mu,
                             this->getDualGrad(RHS));
    }
    ...
    alpha = ls_impl(lsmode, ObjScale, Mu,PrimObj, BarrObj,
        XSL, DXSL,
        Temp, RHS, RHS2,
        Citer, iters);
...
Eigen::VectorXd ASSET::PSIOPT::init_impl(const Eigen::VectorXd& x, double Mu,
                                         bool docompute) {
  ...
  this->evalNLP(AlgorithmModes::INIT, this->ObjScale, XSL, val,
                RHS.head(this->PrimalVars), RHS, this->KKTSol.getMatrix());
  ...
  if (this->PrintLevel < 2) {
    auto cyan = fmt::fg(fmt::color::cyan);
    if (docompute) {
      fmt::print(" LDLT Factor NNZs      : "); fmt::print(cyan,"{0:<10}\n",this->FactorMem);
      fmt::print(" LDLT Factor FLOPs     : "); fmt::print(cyan, "{0} MFLOPs\n", this->FactorFlops);
    }
    fmt::print(" Analysis/Reorder Time : "); fmt::print(cyan, "{0:.3f} ms\n", pretime *1000);
    print_Finished("KKT-Matrix Analysis ");
  }

  Eigen::VectorXd dx = -this->KKTSol.solve(RHS);

  if (EqualCons > 0) this->getEqLmults(XSL) = this->getEqLmults(dx);
  if (this->InequalCons > 0) this->nlp->setSlackDiags(0.0);
  this->nlp->setPrimalDiags(0.0);
...
double ASSET::PSIOPT::ls_impl(LineSearchModes lsmode, double ObjScale,double Mu,
    double PrimObj,double BarrObj,
    EigenRef<VectorXd> XSL, EigenRef<VectorXd> DXSL,
    ...
    if (lsmode == LineSearchModes::LANG) {
        double LangInit =
            PrimObj + BarrObj + this->getLmults(XSL).dot(this->getAllCons(RHS));
        ...
        for (int j = 0; j < this->MaxLSIters; j++) {
            ...
            this->barrier_gradient(this->getSlacks(XSL2), this->getIqLmults(XSL2),
                Mu, this->getDualGrad(RHS2));
            double LangTest =
                ptest + btest + this->getLmults(XSL2).dot(this->getAllCons(RHS2));
            Citer.LSiters = j;
        ...
        double vv = RHS.head(this->PrimalVars + this->SlackVars)
            .dot(DXSL.head(this->PrimalVars + this->SlackVars));
        double cv = this->getLmults(DXSL).dot(this->getAllCons(RHS));

        double LangInit = PrimObj + BarrObj;
        double InitL1Pen =
            this->getLmults(XSL).cwiseAbs().dot(this->getAllCons(RHS).cwiseAbs());
        double InitL2Pen = this->getAllCons(RHS).squaredNorm();

        double sc = .1 + std::abs(vv - cv) / InitL2Pen;
        if (InitL2Pen == 0.0)sc = 1.0;
        ...
        for (int j = 0; j < this->MaxLSIters; j++) {
            ...
            RHS2.setZero();
            this->nlp->evalOCC(
                ObjScale, XSL2.head(this->PrimalVars), ptest,
            ...
            double TestL1Pen = this->getLmults(XSL).cwiseAbs().dot(
                this->getAllCons(RHS2).cwiseAbs());
...
void ASSET::PSIOPT::Build(py::module& m) {
  ...
  obj.def_readwrite("FastFactorAlg", &PSIOPT::FastFactorAlg,
                    PSIOPT_FastFactorAlg);
  ...
  obj.def_readwrite("PDStepStrategy", &PSIOPT::PDStepStrategy,
                    PSIOPT_PDStepStrategy);
  obj.def_readwrite("SOEboundRelax", &PSIOPT::SOEboundRelax,
                    PSIOPT_SOEboundRelax);
  ...
  obj.def_readwrite("ForceQPanalysis", &PSIOPT::ForceQPanalysis,
                    PSIOPT_ForceQPanalysis);
  obj.def_readwrite("QPRefSteps", &PSIOPT::QPRefSteps, PSIOPT_QPRefSteps);

  obj.def_readwrite("QPPivotPerturb", &PSIOPT::QPPivotPerturb,
                    PSIOPT_QPPivotPerturb);
  obj.def_readwrite("QPThreads", &PSIOPT::QPThreads, PSIOPT_QPThreads);
  obj.def_readwrite("QPPivotStrategy", &PSIOPT::QPPivotStrategy,
                    PSIOPT_QPPivotStrategy);
