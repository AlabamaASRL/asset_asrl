...
struct ConstraintFunction : SolverFunctionBase<ConstraintInterface> {
  ...
  ConstraintFunction(const ConstraintInterface& f,
  ...
  */
  ...
  Passes the arguments from PSIOPT and NonLinearProgram as well as the indexing data struct to the underlying
  vector function.
  */
  void constraints(ConstEigenRef<Eigen::VectorXd> X,
                   Eigen::Ref<Eigen::VectorXd> FX) const {
  ...
  */
  void constraints_jacobian(
      ConstEigenRef<Eigen::VectorXd> X, EigenRef<Eigen::VectorXd> FX,
      Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat,
      EigenRef<Eigen::VectorXi> KKTLocations,
      EigenRef<Eigen::VectorXi> KKTClashes,
      std::vector<std::mutex>& KKTLocks) const {
    this->function.constraints_jacobian(X, FX, KKTmat, KKTLocations, KKTClashes,
                                        KKTLocks, this->index_data);
  }
  ...
  */
  void constraints_jacobian_adjointgradient(
    ...
    this->function.constraints_jacobian_adjointgradient(
        X, L, FX, AGX, KKTmat, KKTLocations, KKTClashes, KKTLocks,
        this->index_data);
  }

  /*
  Interface for calling the underlying type erased function's .constraints_jacobian_adjointgradient_adjointhessian method.
  Passes the arguments from PSIOPT and NonLinearProgram as well as the indexing data struct to the underlying
  vector function.
  */
