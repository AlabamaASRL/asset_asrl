    ...
    auto analyzeOP = [&](auto& SourceFuncs, auto& TargetThrFuncs) {
        for (auto& func : SourceFuncs) {
            if (func.getThreadMode() ==
                ThreadingFlags::MainThread) {  // Force to main thread
                ...
                if (RRThr > (this->Threads - 1)) RRThr = 0;
            }
...
{
    this->KKTdim = this->PrimalVars + this->SlackVars + this->EqualCons +
        this->InequalCons;
...
{
    this->numRHSElems = this->numPGXElems + this->numAGXElems +
        this->numEConElems + this->numIConElems;
...
void ASSET::NonLinearProgram::finalizeData()
{
    ...
    for (int i = 0; i < this->InequalCons; i++) {
        ...
        this->SlackCoeffRows()[i] =
            this->PrimalVars + this->SlackVars + this->EqualCons + i;

        this->SlackDiagCoeffCols()[i] = this->PrimalVars + i;
        this->SlackDiagCoeffRows()[i] = this->PrimalVars + i;

        this->IPivotCoeffCols()[i] =
            this->PrimalVars + this->SlackVars + this->EqualCons + i;
        this->IPivotCoeffRows()[i] =
            this->PrimalVars + this->SlackVars + this->EqualCons + i;
    }
}
...
void ASSET::NonLinearProgram::analyzeSparsity(Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat)
{
    ...
    */
    ...
    std::vector<Eigen::Triplet<double>> kktvec(
        this->numKKTElems, Eigen::Triplet<double>(0, 0, 0.0));
...
void ASSET::NonLinearProgram::evalRHS(
  ...
  auto RHSevalOP = [&](int id, int thrnum) {
    ...
    for (auto& Con : this->ThrEq[thrnum])
      Con.constraints_adjointgradient(X, LE, this->EConCoeffs(),
                                      this->AGXCoeffs());
    for (auto& Con : this->ThrIq[thrnum])
      Con.constraints_adjointgradient(X, LI, this->IConCoeffs(),
                                      this->AGXCoeffs());
  };
  ...
  for (int i = 0; i < Thrmin1; i++) results[i].get();
  for (int i = 0; i < this->Threads; i++) val += Vals[i];

  this->fillRHS(PGX, AGX, FXE, FXI);
}

void ASSET::NonLinearProgram::evalOGC(double ObjScale,
                                      ConstEigenRef<VectorXd> X, double& val,
...
void ASSET::NonLinearProgram::evalOCC(double ObjScale,
                                      ConstEigenRef<VectorXd> X, double& val,
                                      EigenRef<VectorXd> FXE,
                                      EigenRef<VectorXd> FXI) {
  int Thrmin1 = this->Threads - 1;
...
void ASSET::NonLinearProgram::evalOBJ(double ObjScale,
    ConstEigenRef<VectorXd> X, double& val) {
    int Thrmin1 = this->Threads - 1;
    std::vector<std::future<void>> results(Thrmin1);
    std::vector<double> Vals(this->Threads, 0.0);
...
void ASSET::NonLinearProgram::evalKKT(
    double ObjScale, ConstEigenRef<VectorXd> X, ConstEigenRef<VectorXd> LE,
    ConstEigenRef<VectorXd> LI, double& val, EigenRef<VectorXd> PGX,
    EigenRef<VectorXd> AGX, EigenRef<VectorXd> FXE, EigenRef<VectorXd> FXI,
  ...
  auto KKTevalOP = [&](int id, int thrnum) {
    for (auto& Obj : this->ThrObj[thrnum])
      Obj.objective_gradient_hessian(
          ObjScale, X, Vals[thrnum], this->PGXCoeffs(), KKTmat,
          this->KKTLocations, this->KKTClashes, this->KKTLocks);
    for (auto& Con : this->ThrEq[thrnum])
      Con.constraints_jacobian_adjointgradient_adjointhessian(
          X, LE, this->EConCoeffs(), this->AGXCoeffs(), KKTmat,
          this->KKTLocations, this->KKTClashes, this->KKTLocks);
    for (auto& Con : this->ThrIq[thrnum])
      Con.constraints_jacobian_adjointgradient_adjointhessian(
          X, LI, this->IConCoeffs(), this->AGXCoeffs(), KKTmat,
          this->KKTLocations, this->KKTClashes, this->KKTLocks);
  };
...
void ASSET::NonLinearProgram::evalKKTNO(
    double ObjScale, ConstEigenRef<VectorXd> X, ConstEigenRef<VectorXd> LE,
    ConstEigenRef<VectorXd> LI, double& val, EigenRef<VectorXd> PGX,
    EigenRef<VectorXd> AGX, EigenRef<VectorXd> FXE, EigenRef<VectorXd> FXI,
    Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat) {
...
void ASSET::NonLinearProgram::evalSOE(
    double ObjScale, ConstEigenRef<VectorXd> X, ConstEigenRef<VectorXd> LE,
    ConstEigenRef<VectorXd> LI, double& val, EigenRef<VectorXd> PGX,
    EigenRef<VectorXd> AGX, EigenRef<VectorXd> FXE, EigenRef<VectorXd> FXI,
    Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat) {
  ...
  auto SOEevalOP = [&](int id, int thrnum) {
    for (auto& Con : this->ThrEq[thrnum])
      Con.constraints_jacobian(X, this->EConCoeffs(), KKTmat,
                               this->KKTLocations, this->KKTClashes,
                               this->KKTLocks);
    for (auto& Con : this->ThrIq[thrnum])
      Con.constraints_jacobian(X, this->IConCoeffs(), KKTmat,
                               this->KKTLocations, this->KKTClashes,
                               this->KKTLocks);
  };
...
void ASSET::NonLinearProgram::evalAUG(
    double ObjScale, ConstEigenRef<VectorXd> X, ConstEigenRef<VectorXd> LE,
    ConstEigenRef<VectorXd> LI, double& val, EigenRef<VectorXd> PGX,
    EigenRef<VectorXd> AGX, EigenRef<VectorXd> FXE, EigenRef<VectorXd> FXI,
    Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat) {
...
void ASSET::NonLinearProgram::NLPTest(const Eigen::VectorXd& x, int n,
  ...
  Eigen::SparseMatrix<double, Eigen::RowMajor> KKTmat1(nlp1->KKTdim,
                                                       nlp1->KKTdim);
  Eigen::SparseMatrix<double, Eigen::RowMajor> KKTmat2(nlp1->KKTdim,
                                                       nlp1->KKTdim);
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (i%10 == 0) {
      ...
      Eigen::SparseMatrix<double, Eigen::RowMajor> mat =
          (KKTmat1 - KKTmat2).cwiseAbs();
      for (int k = 0; k < mat.outerSize(); ++k)
        for (Eigen::SparseMatrix<double, Eigen::RowMajor>::InnerIterator it(mat,
                                                                            k);
             it; ++it) {
          it.value();
          if (it.value() > maxval) {
