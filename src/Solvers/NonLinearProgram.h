...
struct NonLinearProgram {
  ...
  NonLinearProgram(int PV, int EQ, int IQ, std::vector<ObjectiveFunction>& obj,
                   ...
                   std::vector<ConstraintFunction>& ineq, int FunThr) {
    this->Threads = std::max(FunThr, 1);
    this->TP.resize(this->Threads + 2);
  ...
  EigenRef<VectorXd> PrimalDiagCoeffs() {
    return this->SolverCoeffs.segment(this->PrimalDiagsDataStart,
                                      this->PrimalVars);
  }
  EigenRef<VectorXd> SlackDiagCoeffs() {
    return this->SolverCoeffs.segment(this->SlackDiagDataStart,
                                      this->SlackVars);
  }
  ...
  EigenRef<VectorXi> SlackCoeffCols() {
    return this->KKTcoeffCols.segment(
        this->SlackJacDataStart + this->numUserKKTElems, this->SlackVars);
  }
  EigenRef<VectorXi> PrimalDiagCoeffCols() {
    return this->KKTcoeffCols.segment(
        this->PrimalDiagsDataStart + this->numUserKKTElems, this->PrimalVars);
  }
  EigenRef<VectorXi> SlackDiagCoeffCols() {
    return this->KKTcoeffCols.segment(
        this->SlackDiagDataStart + this->numUserKKTElems, this->SlackVars);
  }
  EigenRef<VectorXi> EPivotCoeffCols() {
    return this->KKTcoeffCols.segment(
        this->EPivotDataStart + this->numUserKKTElems, this->EqualCons);
  }
  EigenRef<VectorXi> IPivotCoeffCols() {
    return this->KKTcoeffCols.segment(
        this->IPivotDataStart + this->numUserKKTElems, this->InequalCons);
  }

  EigenRef<VectorXi> SlackCoeffRows() {
    return this->KKTcoeffRows.segment(
        this->SlackJacDataStart + this->numUserKKTElems, this->SlackVars);
  }
  ...
  void assignKKTSlackHessian(
      const Eigen::Ref<const Eigen::VectorXd>& slhs,
  ...
  void perturbKKTPDiags(double pert,
                        Eigen::SparseMatrix<double, Eigen::RowMajor>& mat) {
  ...
  void fillRHS(EigenRef<VectorXd> PGX, EigenRef<VectorXd> AGX,
  ...
  void evalRHS(double ObjScale, ConstEigenRef<VectorXd> X,
               ConstEigenRef<VectorXd> LE, ConstEigenRef<VectorXd> LI,
               double& val, EigenRef<VectorXd> PGX, EigenRef<VectorXd> AGX,
               EigenRef<VectorXd> FXE, EigenRef<VectorXd> FXI);

  void evalOGC(double ObjScale, ConstEigenRef<VectorXd> X, double& val,
               EigenRef<VectorXd> PGX, EigenRef<VectorXd> FXE,
               EigenRef<VectorXd> FXI);

  void evalOCC(double ObjScale, ConstEigenRef<VectorXd> X, double& val,
               EigenRef<VectorXd> FXE, EigenRef<VectorXd> FXI);

  void evalOBJ(double ObjScale, ConstEigenRef<VectorXd> X, double& val);

  void evalKKT(double ObjScale, ConstEigenRef<VectorXd> X,
               ConstEigenRef<VectorXd> LE, ConstEigenRef<VectorXd> LI,
               double& val, EigenRef<VectorXd> PGX, EigenRef<VectorXd> AGX,
               EigenRef<VectorXd> FXE, EigenRef<VectorXd> FXI,
               Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat);

  void evalKKTNO(double ObjScale, ConstEigenRef<VectorXd> X,
                 ConstEigenRef<VectorXd> LE, ConstEigenRef<VectorXd> LI,
                 double& val, EigenRef<VectorXd> PGX, EigenRef<VectorXd> AGX,
                 EigenRef<VectorXd> FXE, EigenRef<VectorXd> FXI,
                 Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat);

  void evalSOE(double ObjScale, ConstEigenRef<VectorXd> X,
               ConstEigenRef<VectorXd> LE, ConstEigenRef<VectorXd> LI,
               double& val, EigenRef<VectorXd> PGX, EigenRef<VectorXd> AGX,
               EigenRef<VectorXd> FXE, EigenRef<VectorXd> FXI,
               Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat);

  void evalAUG(double ObjScale, ConstEigenRef<VectorXd> X,
               ConstEigenRef<VectorXd> LE, ConstEigenRef<VectorXd> LI,
               double& val, EigenRef<VectorXd> PGX, EigenRef<VectorXd> AGX,
               EigenRef<VectorXd> FXE, EigenRef<VectorXd> FXI,
               Eigen::SparseMatrix<double, Eigen::RowMajor>& KKTmat);
