...
namespace ASSET {
	...
	thread_local MemoryManager::SuperScalarStackType
		MemoryManager::SuperScalarStack
		= detail::TempStack<DefaultSuperScalar>(ASSET_DEFAULT_ARENA_SIZE);
	thread_local MemoryManager::ScalarStackType
		MemoryManager::ScalarStack
	...
	void MemoryManager::Build(py::module& m) {
		...
		obj.def_static("enable_arena_memory", 
			[]() {
				MemoryManager::enable_arena_memory();
			});
		obj.def_static("disable_arena_memory",
			[]() {
