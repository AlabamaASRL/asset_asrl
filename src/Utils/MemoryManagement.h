...
namespace ASSET {
	...
	namespace detail {
		...
		struct TempStack {
			...
			inline void freeBlock(int blocksize) {
				if (OverFlowSize == 0) NextStart -= blocksize;
				else OverFlowSize -= blocksize;
				if (NextStart == 0 && OverFlowSize == 0 && (NextDataSize != DataSize)) {
					DataSize = NextDataSize;
					this->Data.resize(DataSize);
					if(OverFlowData.size()>1)OverFlowData.clear();
				}
			}
			inline int size() {
				return Data.size();
		...
		struct NJumpTable {
			...
			static void run(Ftype&& f, int crit_size) {
				...
				if (!done) f(std::integral_constant<int, -1>());
	...
	struct TupleOfTempSpecs {
		...
		using Scalar = typename std::remove_const_reference<decltype(std::get<0>(std::tuple<T...>())) > ::type::Scalar;
	...
	struct MemoryManager {
		...
		static void allocate_run_impl(const JTable & jt, int critical_size, Func&& f, const TempSpecs & ... tspecs) {
			if constexpr ((... && TempSpecs::IsConstantSize)) {
				...
				if constexpr (OnlyOldTables) MemoryManager::run_old_table_impl( jt, critical_size, tspecs...);
				else if constexpr (OnlyNewTables) MemoryManager::run_new_table_impl(jt,f, tspecs...);
				else if constexpr (OnlyArena) MemoryManager::run_arena_impl(f, tspecs...);
				else {
					if (MemoryManager::UseArena) MemoryManager::run_arena_impl(f, tspecs...);
					else MemoryManager::run_new_table_impl(jt,f, tspecs...);
				}
			}
		}
	...
	private:
		...
		using DefaultJumpTable = typename std::conditional< OnlyOldTables,
			detail::OldDefaultJumpTable,
		...
		inline static int count_blocksize(const TempSpecs & ... tspecs) {
			int blksize = 0;
			auto CalcSpecSize = [](const auto & tspec) {
				return (tspec.rows * tspec.cols);
			};
		...
		inline static auto make_temps(Scalar * data, const TempSpecs & ... tspecs) {
			...
			auto make_temp = [&](const auto& tspec) {
				...
				if constexpr (type::IsConstantSize) {
					...
					auto array_temp = [&](auto i) {
						return make_map(tspec.tspec);
					};
		...
		inline static void run_arena_impl(Func&& f, const TempSpecs & ... tspecs) {
			using Scalar = typename std::remove_const_reference<decltype(std::get<0>(std::tuple{ tspecs... })) > ::type::Scalar;
		...
		template<class JTable, class Func, class ... TempSpecs>
		inline static void run_old_table_impl(const JTable& jt, int critical_size, Func&& f, const TempSpecs & ... tspecs) {
			auto MaxImpl = [&](auto maxsize) {
				auto Temps = detail::MaxTempPack<maxsize.value, typename std::remove_const_reference<TempSpecs>::type ...>(tspecs ...);
				std::apply(f, Temps.data);
			};
