...
template <typename TupleType, typename FunctionType>
void reverse_tuple_for_each(TupleType&&, FunctionType,
                            std::integral_constant<size_t, 0>) {}
...
void reverse_tuple_for_each(TupleType&& t, FunctionType f,
  ...
  reverse_tuple_for_each(std::forward<TupleType>(t), f,
                         std::integral_constant<size_t, I - 1>());
}
template <typename TupleType, typename FunctionType>
void reverse_tuple_for_each(TupleType&& t, FunctionType f) {
...
void constexpr const_tuple_for_each(TupleType&& t, FunctionType f,
                                    std::integral_constant<size_t, I>) {
  f(std::get<I>(std::forward<TupleType>(t)));
  const_tuple_for_each(std::forward<TupleType>(t), f,
                       std::integral_constant<size_t, I + 1>());
}
...
void tuple_for_loop(TupleType&& t, FunctionType f,
  ...
  f(std::get<I>(std::forward<TupleType>(t)),
    std::integral_constant<size_t, I>());
  tuple_for_loop(std::forward<TupleType>(t), f,
                 std::integral_constant<size_t, I + 1>());
}
template <typename TupleType, typename FunctionType>
void tuple_for_loop(TupleType&& t, FunctionType f) {
...
template <typename TupleType, typename FunctionType>
void reverse_tuple_for_loop(TupleType&&, FunctionType,
                            std::integral_constant<size_t, 0>) {}
...
void reverse_tuple_for_loop(TupleType&& t, FunctionType f,
  ...
  f(std::get<I - 1>(std::forward<TupleType>(t)),
    std::integral_constant<size_t, I - 1>());
  reverse_tuple_for_loop(std::forward<TupleType>(t), f,
                         std::integral_constant<size_t, I - 1>());
}
template <typename TupleType, typename FunctionType>
void reverse_tuple_for_loop(TupleType&& t, FunctionType f) {
...
auto constexpr_forwarding_loop_impl(FunctionType f, RetType input) {
  if constexpr (I < F) {
    ...
    return constexpr_forwarding_loop_impl<I + 1, F, FunctionType,
                                          decltype(output)>(f, output);
  } else
    return input;
}
...
auto constexpr_forwarding_loop(std::integral_constant<int, I>,
                               std::integral_constant<int, F>, FunctionType f,
                               RetType input) {
