 ...
 ********************************************************
 ...
 This file has been modified from its original version by renaming the original thread_pool class to ThreadPool and
 adding the push_member function to the class. 
 */
...
class ThreadPool {
 public:
  ...
  auto push_member(F&& f, Obj* obj, Rest&&... rest)
      -> std::future<decltype(f(obj, 0, rest...))> {
    auto pck =
        std::make_shared<std::packaged_task<decltype(f(obj, 0, rest...))(int)>>(
            std::bind(std::forward<F>(f), obj, std::placeholders::_1,
                      std::forward<Rest>(rest)...));
    auto _f = new std::function<void(int id)>([pck](int id) { (*pck)(id); });
    this->q.push(_f);
    std::unique_lock<std::mutex> lock(this->mutex);
