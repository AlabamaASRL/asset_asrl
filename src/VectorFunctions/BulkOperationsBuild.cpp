...
void ASSET::BulkOperationsBuild(FunctionRegistry& reg, py::module& m) {
  ...
  m.def("stack",
      [](const std::vector<GenS>& elems) {
          return DynamicStack(elems);
      });
  m.def("stack",
      [](const std::vector<Gen>& elems) {
          return DynamicStack(elems);
      });
  m.def("stack",
      [](const Gen & first, py::args x) {
  ...
  m.def("stack",
      [](Eigen::VectorXd first, py::args x) {
