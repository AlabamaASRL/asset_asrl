...
File Description: Defines the class GenericFunction which is a vector function that can be constructed form ANY
other dense vector function with compatible input and output sizes. This is probably the most important class in the vector
functions part of the library as it allows us to type erase arbitrarily compilicated compile time or run time defined vector
functions. It holds type easure object that constructable from any compatible object and forwards its compute calls as well as
selected product and accumulation operations to this type erased function. 
...
namespace ASSET {
    ...
    namespace detail {
        ...
        */
        ...
        struct GFDeepCopySelector {
            using type = DeepCopySpecs<GFTE<IR, OR>, GFTE<-1, -1>,
                ConstraintInterface>;
        };
        template <int IR>
        struct GFDeepCopySelector<IR, 1> {
            using type = DeepCopySpecs<GFTE<IR, 1>, GFTE<-1, -1>,
        ...
        template<int IR, int OR>
        using GenTE = rubber_types::TypeErasure<
            typename GFDeepCopySelector<IR, OR>::type,
	...
	struct GenericFunction:VectorFunction<GenericFunction<IR,OR>,IR,OR> {
        ...
        void accumulate_hessian(ConstMatrixBaseRef<Target> target_,
            ...
            typedef typename Target::Scalar Scalar;
            if (!this->is_linear()) Base::accumulate_hessian(target_, right, assign);
        }
        ...
        void scale_hessian(ConstMatrixBaseRef<AdjHessType> target_,
            Scalar s) const {
            if (!this->is_linear())Base::scale_hessian(target_, s);
        ...
        void SpeedTest(const Input<double> & xt, int n) {
            ...
            std::cout << "  SuperScalar: " << MemoryManager::size_super_scalar()*8* ASSET::DefaultSuperScalar::SizeAtCompileTime << std::endl;
