...
struct DenseFunctionSpec {
  ...
  using JacType = typename std::conditional<OR == 1, 
  ...
  using SuperJacType = typename std::conditional<
      OR == 1,
      Eigen::Ref<Eigen::Matrix<ASSET::DefaultSuperScalar, -1, IR>, 0,
                 Eigen::Stride<-1, -1>>,
  ...
  struct Concept {  // abstract base class for model.
    ...
    virtual void scale_jacobian(ConstMatrixBaseRef<JacType> target_,
                                double s) const = 0;
   ...
   /* virtual void accumulate_gradient(ConstMatrixBaseRef<AdjGradType> target_,
    ...
    virtual void right_jacobian_product(
        ...
        ConstEigenBaseRef<LeftJacMatrix> left, ConstEigenBaseRef<JacType> right,
        DirectAssignment assign, std::bool_constant<true> aliased) const = 0;
    virtual void right_jacobian_product(
        ConstMatrixBaseRef<RightJacTarget> target_,
        ConstEigenBaseRef<LeftJacMatrix> left, ConstEigenBaseRef<JacType> right,
    ...
    virtual void right_jacobian_product(
        ConstMatrixBaseRef<RightJacTarget> target_,
        ConstEigenBaseRef<LeftJacMatrix> left, ConstEigenBaseRef<JacType> right,
        DirectAssignment assign, std::bool_constant<false> aliased) const = 0;
    virtual void right_jacobian_product(
        ConstMatrixBaseRef<RightJacTarget> target_,
        ConstEigenBaseRef<LeftJacMatrix> left, ConstEigenBaseRef<JacType> right,
        PlusEqualsAssignment assign,
  ...
  struct Model : public Holder, public virtual Concept {
    ...
    virtual void compute(ConstVectorBaseRef<InType> x,
                         ConstVectorBaseRef<OutType> fx_) const override {
    ...
    virtual void accumulate_jacobian(ConstMatrixBaseRef<JacType> target_,
      ...
      return rubber_types::model_get(this).accumulate_jacobian(target_, right,
                                                               assign);
    }
    virtual void accumulate_jacobian(
        ConstMatrixBaseRef<JacType> target_, ConstMatrixBaseRef<JacType> right,
  ...
  struct ExternalInterface : public Container {
    ...
    */
    ...
    void compute(ConstVectorBaseRef<InTypeTT> x,
    ...
    template <class InTypeTT, class OutTypeTT, class JacTypeTT,
              class AdjGradTypeTT, class AdjHessTypeTT, class AdjVarTypeTT>
    ...
    void accumulate_jacobian(ConstMatrixBaseRef<TargetTT> target_,
      ...
      return rubber_types::interface_get(this).accumulate_jacobian(
          jtarg, jright, assign);
    }
