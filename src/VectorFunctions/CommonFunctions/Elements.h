...
struct Elements
    : VectorFunction<Elements<IR, EL1, ELS...>, IR, 1 + sizeof...(ELS)> {
  using Base =
      VectorFunction<Elements<IR, EL1, ELS...>, IR, 1 + sizeof...(ELS)>;
  DENSE_FUNCTION_BASE_TYPES(Base);
  using Base::compute;
  static const std::tuple<std::integral_constant<int, EL1>,
  ...
  inline void compute_impl(ConstVectorBaseRef<InType> x,
    ...
    ASSET::tuple_for_loop(
        elements, [&](const auto& ele, auto i) { fx[i] = x[ele.value]; });
  }
