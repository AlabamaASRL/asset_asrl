...
namespace ASSET {
    ...
    struct NestedCallAndAppendChain2
        : VectorFunction<
        NestedCallAndAppendChain2<OuterFunc, InnerFunc1, InnerFuncs...>,
        ...
        NestedCallAndAppendChain2(OuterFunc outer_func,
            ...
            ASSET::constexpr_for_loop(
                ...
                std::integral_constant<int, sizeof...(InnerFuncs)>(), [&](auto i) {
                    std::get<i.value>(this->inner_funcs) =
                        std::get<i.value + 1>(inner_funct);
                });
        ...
        inline void compute_jacobian_impl(ConstVectorBaseRef<InType> x,
            ...
            auto Impl = [&](auto& xchain,auto & jx1,auto & jxi,auto & jxO) {
                ...
                this->inner_func1.compute_jacobian(
                    x,
                    xchain.template segment<InnerFunc1::ORC>(this->IRows(),
                ...
                ASSET::reverse_tuple_for_loop(this->inner_funcs, [&](const auto& func_i,
                    auto i) {
                        using FTtype = typename std::remove_const<
                            typename std::remove_reference<decltype(func_i)>::type>::type;
                        func_i.right_jacobian_product(
                            ...
                            std::get<i.value>(jxi), PlusEqualsAssignment(),
            ...
            auto make_temp_tuple = [&](auto f) {
                auto app = [&](const auto & ... func_i) {
                    return std::tuple{ f(func_i)... };
                };
            ...
            auto JITemps = TupleOfTempSpecs<typename InnerFuncs::template Jacobian<Scalar>...>{ make_temp_tuple(jis) };
        ...
        template <class InType, class OutType, class JacType, class AdjGradType,
            ...
            auto Impl = [&](auto& xchain, 
                auto& jx1,auto & gx1,auto & hx1, 
                auto& jxi,auto & gxi,auto & hxi,
                auto& jxO,auto & gxO,auto & hxO,
                auto & j0s) {
                ...
                this->outer_func.compute_jacobian_adjointgradient_adjointhessian(
                    xchain, fx_, jxO, gxO, hxO, adjvars);

                ASSET::reverse_tuple_for_loop(this->inner_funcs,
                    [&](const auto& func_i, auto i) {
                        using FTtype = typename std::remove_const<
                            typename std::remove_reference<decltype(func_i)>::type>::type;
                        ...
                        func_i.compute_jacobian_adjointgradient_adjointhessian(
                            xchain.template head<FTtype::IRC>(func_i.IRows()),
                            xchain.template segment<FTtype::ORC>(func_i.IRows(), func_i.ORows()),
                            std::get<i.value>(jxi), std::get<i.value>(gxi),
                            std::get<i.value>(hxi),
                ...
                ASSET::tuple_for_loop(this->inner_funcs, [&](const auto& func_i, auto i) {
                    ...
                    j0s.template middleRows<FTtype::ORC>(func_i.IRows(), func_i.ORows()) =
                        ...
                        std::get<i.value>(jxi).template rightCols<Ev>(
                            func_i.IRows() - this->IRows()) *
                        j0s.template middleRows<Ev>(
                ...
                ASSET::tuple_for_loop(this->inner_funcs, [&](const auto& func_i, auto i) {
                    ...
                    func_i.accumulate_hessian(
                        hxO.template topLeftCorner<FTtype::IRC, FTtype::IRC>(
            ...
            auto JITemps = TupleOfTempSpecs<typename InnerFuncs::template Jacobian<Scalar>...>{ make_temp_tuple(jis) };
            auto GITemps = TupleOfTempSpecs<typename InnerFuncs::template Gradient<Scalar>...>{ make_temp_tuple(gis) };
            auto HITemps = TupleOfTempSpecs<typename InnerFuncs::template Hessian<Scalar>...>{ make_temp_tuple(his) };
            ...
            MemoryManager::allocate_run(this->outer_func.IRows(), Impl,
                ...
                TempSpec< Eigen::Matrix<Scalar, OuterFunc::IRC, Base::IRC>>(this->outer_func.IRows(), this->IRows())
            );
...
struct NestedCallAndAppendChain
    : VectorFunction<
          NestedCallAndAppendChain<OuterFunc, InnerFunc1, InnerFuncs...>,
          InnerFunc1::IRC, OuterFunc::ORC> {
  using Base = VectorFunction<
      NestedCallAndAppendChain<OuterFunc, InnerFunc1, InnerFuncs...>,
      InnerFunc1::IRC, OuterFunc::ORC>;
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian_impl(
    ...
    ASSET::reverse_tuple_for_loop(this->inner_funcs, [&](const auto &func_i,
      ...
      if constexpr (ReverseAlg) {
        ...
        func_i.right_jacobian_product(
            jt,
            hxO.template block<FTtype::ORC, FTtype::ORC>(func_i.IRows(),
                                                         func_i.IRows()),
        ...
        func_i.right_jacobian_product(
            hxO.template topLeftCorner<FTtype::IRC, FTtype::IRC>(
                func_i.IRows(), func_i.IRows()),
            jt.transpose(), std::get<i.value>(jxi), PlusEqualsAssignment(),
            std::bool_constant<false>());
    ...
    if constexpr (ReverseAlg) {
      ...
      inner_func1.right_jacobian_product(
          jt,
          hxO.template block<FTtype::ORC, FTtype::ORC>(inner_func1.IRows(),
                                                       inner_func1.IRows()),
      ...
      this->inner_func1.right_jacobian_product(
          hx1,
          hxO.template block<InnerFunc1::IRC, InnerFunc1::ORC>(
              0, inner_func1.IRows()),
          jx1, DirectAssignment(), std::bool_constant<false>());
      ...
      adjhess.noalias() +=
          j0s.template bottomRows<Ev>().transpose() *
