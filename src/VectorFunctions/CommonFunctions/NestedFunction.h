...
struct NestedFunction
    : NestedFunction_Impl<NestedFunction<OuterFunc, InnerFunc>, OuterFunc,
                          InnerFunc> {
  using Base = NestedFunction_Impl<NestedFunction<OuterFunc, InnerFunc>,
                                   OuterFunc, InnerFunc>;
...
template <class Derived, class OuterFunc, class InnerFunc>
struct NestedFunction_Impl
    : VectorFunction<Derived, InnerFunc::IRC, OuterFunc::ORC> {
  ...
  static const bool IsLinearFunction =
      OuterFunc::IsLinearFunction && InnerFunc::IsLinearFunction;
  ...
  NestedFunction_Impl(OuterFunc ofunc, InnerFunc ifunc)
    ...
    if (this->inner_func.ORows() != this->outer_func.IRows()) {
      ...
      fmt::print(fmt::fg(fmt::color::red),
          ...
          "Output Size of InnerFunction (ORows = {0:}) does not match Input Size of OuterFunction (IRows = {1:}).\n",
  ...
  inline void compute_jacobian_impl(ConstVectorBaseRef<InType> x,
    ...
    if constexpr (Is_Segment<OuterFunc>::value) {
        ...
        auto Impl = [&](auto& fx_inner, auto& jx_inner) {
            ...
            fx = fx_inner.template segment<OuterFunc::ORC>(this->outer_func.SegStart, this->ORows());
            this->inner_func.accumulate_matrix_domain(jx_, 
        ...
        auto Impl = [&](auto& fx_inner, auto& jx_inner, auto& jx_outer) {
            ...
            this->inner_func.right_jacobian_product(jx_, jx_outer, jx_inner,
                DirectAssignment(),
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian_impl(
    ...
    if constexpr (Is_Segment<OuterFunc>::value) {
        ...
        auto Impl = [&](auto& fx_inner, auto& jx_inner, auto& jx_outer,
            auto& gx_outer, auto& hx_outer, auto& Ht) {
                this->inner_func.compute(x, fx_inner);
                ...
                if constexpr (!OuterFunc::IsLinearFunction) {
                    ...
                    this->inner_func.right_jacobian_product(Ht, hx_outer, jx_inner,
                        DirectAssignment(),
                        std::bool_constant<false>());
                    if constexpr (InnerFunc::InputIsDynamic) {
                        ...
                        Eigen::Matrix<Scalar, InnerFunc::IRC, InnerFunc::ORC> HTT =
                            Ht.transpose();
  ...
  inline void accumulate_hessian(ConstMatrixBaseRef<Target> target_,
    ...
    if constexpr (OR > 0 && IR > 0 && OuterFunc::IRC == -1) {
      ...
      this->outer_func.accumulate_hessian(
          target.template block<InnerFunc::ORC, InnerFunc::ORC>(
