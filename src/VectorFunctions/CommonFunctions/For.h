...
template <class Derived, int N, class StartFunc, class BodyFunc>
struct For_Impl
    : NestedFunction_Impl<Derived, decltype(BodyFunc::template Definition<N>()),
  ...
  For_Impl(){};
  For_Impl(StartFunc f)
      : Base::NestedFunction_Impl(OFuncType(), IFuncType(f)){};
};
...
struct For_Impl<Derived, 0, StartFunc, BodyFunc>
    : NestedFunction_Impl<Derived, decltype(BodyFunc::template Definition<0>()),
                          StartFunc> {
