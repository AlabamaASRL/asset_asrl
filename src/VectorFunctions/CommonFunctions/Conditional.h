...
struct IfElseFunction
  ...
  using Base = VectorFunction<IfElseFunction<TestFunc, TrueFunc, FalseFunc>,
                              ...
                              SZ_MAX<TrueFunc::ORC, FalseFunc::ORC>::value>;
  using INPUT_DOMAIN =
      CompositeDomain<Base::IRC, typename TrueFunc::INPUT_DOMAIN,
  ...
  IfElseFunction(TestFunc test, TrueFunc _true, FalseFunc _false)
    ...
    this->set_input_domain(this->IRows(), { this->true_func.input_domain(),
                                           this->false_func.input_domain() });
    if (this->true_func.ORows() != this->false_func.ORows()) {
        throw std::invalid_argument("True and false functions in conditional statement must have same number of outputrows.");
    }
    if (this->true_func.IRows() != this->false_func.IRows()) {
        throw std::invalid_argument("True and false functions in conditional statement must have same number of inputrows.");
    }
    if (this->test_func.IRows() != this->false_func.IRows()) {
        
        throw std::invalid_argument("Test,True,and False functions in conditional statement must have same number of inputrows.");
    }
  }
