...
struct ParsedInput
    : VectorFunction<ParsedInput<Func, IRC, ORC>, IRC, ORC, Analytic> {
  ...
  ParsedInput(Func f, const Func_Input<int>& varlocs, int irr)
    ...
    for (int i = 0; i < varlocs.size() - 1; i++) {
      ...
      if (delta != 1) this->contiguous = false;
    }
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian_impl(
    ...
    if (this->contiguous) {
      this->func.compute_jacobian_adjointgradient_adjointhessian(
          ...
          adjhess.block(varlocs[0], varlocs[0], this->func.IRows(),
                        this->func.IRows()),
          adjvars);
      ...
      this->func.compute_jacobian_adjointgradient_adjointhessian(
          xin, fx_, jxin, gxin, hxin, adjvars);
