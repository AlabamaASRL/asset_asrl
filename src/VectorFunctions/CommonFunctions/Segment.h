...
struct Is_ScaledSegment<StaticScaled<Segment<IR, OR, ST>, VALUE>>
    : std::true_type {};
...
struct Segment_Impl : VectorFunction<Derived, IR, OR>, SegStartHolder<ST> {
  ...
  inline void compute_jacobian_impl(ConstVectorBaseRef<InType> x,
    ...
    jx.template middleCols<OR>(this->SegStart, OROWS)
        .diagonal()
        .setConstant(ONE);
  }
  ...
  inline void right_jacobian_product(
    ...
    auto Impl = [&](auto & diag) {
      ...
      diag = right.derived()
          .template middleCols<OR>(this->SegStart, this->ORows())
          .diagonal();

      if constexpr (std::is_same<Assignment, DirectAssignment>::value) {
        if constexpr (Is_EigenDiagonalMatrix<
                          typename std::remove_const_reference<decltype(
                              left.derived())>::type>::value) {
      ...
      else if constexpr (std::is_same<Assignment, ScaledDirectAssignment<
                                                        Scalar>>::value) {
        target.template middleCols<OR>(this->SegStart, this->ORows())
  ...
  inline void symetric_jacobian_product(
    ...
    if constexpr (std::is_same<Assignment, DirectAssignment>::value) {
      ...
      if constexpr (Is_EigenDiagonalMatrix<typename std::remove_const_reference<
        ...
        target
            .template block<OR, OR>(this->SegStart, this->SegStart,
                                    this->ORows(), this->ORows())
            .diagonal() +=
            left.derived().diagonal().cwiseProduct(diag.diagonal());
      } else {
        target
            .template block<OR, OR>(this->SegStart, this->SegStart,
                                    this->ORows(), this->ORows())
  ...
  decltype(auto) rearged(const DenseFunctionBase<Func, FuncIRC, IR>& f) const {
    return Base::template EVALOP<Func>::make_nested(this->derived(),
                                                    f.derived());
  }
