...
struct CwiseSin : CwiseFunctionOperator<CwiseSin<Func>, Func> {
  ...
  static void cwise_compute(ConstVectorBaseRef<InType> x,
...
struct CwiseArcSinH : CwiseFunctionOperator<CwiseArcSinH<Func>, Func> {
    ...
    static void cwise_compute_jacobian(ConstVectorBaseRef<InType> x,
        ...
        if constexpr (std::is_same<Scalar, DefaultSuperScalar>::value) {
            if constexpr (Base::ORC == 1) jx[0] = sqrt(jx[0]);
            else {
                const int sz = x.size();
                for (int i = 0; i < sz; i++)
                    jx[i] = sqrt(jx[i]);
...
struct CwiseSqrt : CwiseFunctionOperator<CwiseSqrt<Func>, Func> {
  ...
  static void cwise_compute(ConstVectorBaseRef<InType> x,
    ...
    VectorBaseRef<OutType> fx = fx_.const_cast_derived();
    if constexpr (std::is_same<Scalar, DefaultSuperScalar>::value) {
        if constexpr (Base::ORC == 1) fx[0] = sqrt(x[0]);
        else {
            const int sz = x.size();
            for (int i = 0; i < sz; i++)
                fx[i] = sqrt(x[i]);
...
struct CwisePow : CwiseFunctionOperator<CwisePow<Func>, Func> {
    ...
    CwisePow(){}
    template <class InType, class OutType>
    void cwise_compute(ConstVectorBaseRef<InType> x,
...
struct CwiseFunctionOperator : VectorFunction<Derived, Func::IRC, Func::ORC> {
  ...
  inline void compute_jacobian_impl(ConstVectorBaseRef<InType> x,
    ...
    if constexpr (Func::ORC == 1) {
      ...
      this->func.right_jacobian_product(jx_, jxdiag.asDiagonal(), jx_,
                                        DirectAssignment(),
                                        std::bool_constant<true>());
    }
  }
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian_impl(
    ...
    this->func.compute_jacobian_adjointgradient_adjointhessian(
        x, fxt, jx_, adjgrad_, adjhess_, adjtemp);
    if constexpr (Func::ORC == 1) {
       
      this->func.symetric_jacobian_product(adjhess, hxdiag, jx,
      ...
      this->func.symetric_jacobian_product(adjhess, hxdiag.asDiagonal(), jx,
                                           PlusEqualsAssignment(),
                                           std::bool_constant<false>());
      this->func.right_jacobian_product(jx_, jxdiag.asDiagonal(), jx,
                                        DirectAssignment(),
                                        std::bool_constant<true>());
    }
  }
...
struct CwiseOperator : VectorFunction<Derived, IR, IR> {
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian(
    ...
    VectorBaseRef<AdjGradType> adjgrad = adjgrad_.const_cast_derived();
    MatrixBaseRef<AdjHessType> adjhess = adjhess_.const_cast_derived();
    this->derived().cwise_compute_jacobian_hessian(x, fx_, jx.diagonal(),
                                                   adjhess.diagonal());
