...
namespace ASSET {
	...
	struct ScalarRootFinder_Impl:VectorFunction<Derived,FX::IRC,1> {
        ...
        ScalarRootFinder_Impl(FX f , DFX df, int iter,double tol):fxfunc(f),dfxfunc(df),MaxIters(iter),tol(tol) {
        ...
        template <class InType, class OutType, class JacType, class AdjGradType,
            ...
            auto Impl = [&](auto& xtmp,auto & jtmp, auto & htmp) {
                ...
                htmp /= -jx(0, 0);
                adjhess.noalias() = htmp + (jx.transpose() * (jx * htmp(0, 0) / jx(0, 0) - htmp.row(0)) - htmp.col(0) * jx) / jx(0, 0);
