...
struct MatrixFunctionProduct
    : MatrixFunctionProduct_Impl<MatrixFunctionProduct<MatFunc1, MatFunc2>,
                                  MatFunc1, MatFunc2> {
  using Base =
      MatrixFunctionProduct_Impl<MatrixFunctionProduct<MatFunc1, MatFunc2>,
                                  MatFunc1, MatFunc2>;
...
struct MatrixFunctionProduct_Impl
    : VectorFunction<Derived, SZ_MAX<MatFunc1::IRC, MatFunc2::IRC>::value,
                     SZ_PROD<MatFunc1::MROWS, MatFunc2::MCOLS>::value, Analytic,
    Analytic> {
  using Base =
      VectorFunction<Derived, SZ_MAX<MatFunc1::IRC, MatFunc2::IRC>::value,
                     SZ_PROD<MatFunc1::MROWS, MatFunc2::MCOLS>::value, Analytic,
  ...
  using INPUT_DOMAIN =
      CompositeDomain<Base::IRC, typename MatFunc1::INPUT_DOMAIN,
                      typename MatFunc2::INPUT_DOMAIN>;

  static const bool IsVectorizable =
      MatFunc1::IsVectorizable && MatFunc2::IsVectorizable;
  // static const bool IsVectorizable = false;

  DENSE_FUNCTION_BASE_TYPES(Base);

  MatrixFunctionProduct_Impl() {}
  MatrixFunctionProduct_Impl(MatFunc1 mf1, MatFunc2 mf2)
      : matrix_func1(mf1), matrix_func2(mf2) {
    ...
    if (this->matrix_func1.MatrixCols != this->matrix_func2.MatrixRows) {
        throw std::invalid_argument("Invalid matrix product. Number of columns in matrix 1 does not match number of rows in matrix 2.");
    }

    this->setIORows(this->matrix_func1.IRows(), m1rows * m2cols);

    this->set_input_domain(this->IRows(), {matrix_func1.input_domain(),
                                           matrix_func2.input_domain()});
  }
  ...
  inline void compute_impl(ConstVectorBaseRef<InType> x,
    ...
    auto Impl = [&](auto& fxm1, auto& fxm2, auto& fxmd) {
      ...
      Eigen::Map<const MatrixOne<Scalar>> m1(fxm1.data(), m1rows,
                                             m1cols_m2rows);
      Eigen::Map<const MatrixTwo<Scalar>> m2(fxm2.data(), m1cols_m2rows,
                                             m2cols);
  ...
  inline void compute_jacobian_impl(ConstVectorBaseRef<InType> x,
    ...
    ///////////////////////////
    auto Impl = [&](auto& fxm1, auto& jxm1, auto& fxm2, auto& jxm2, auto& fxmd,
                    auto& dvec) {
      ...
      for (int i = 0; i < m2cols; i++) {
        if constexpr (M1Major == Eigen::ColMajor) {
          ...
          for (int j = 0; j < m1rows; j++) {
            this->matrix_func1.right_jacobian_domain_product(
                jx.row(m1rows * i + j), m2.col(i).transpose(),
                jxm1.template middleRows<M1Cols_M2Rows>(m1cols_m2rows * j,
                                                        m1cols_m2rows),
                PlusEqualsAssignment(), std::bool_constant<false>());
          }
        }
        ...
        if constexpr (M2Major == Eigen::ColMajor) {
          this->matrix_func2.right_jacobian_domain_product(
              jx.template middleRows<M1Rows>(m1rows * i, m1rows), m1,
              jxm2.template middleRows<M1Cols_M2Rows>(m1cols_m2rows * i,
                                                      m1cols_m2rows),
              PlusEqualsAssignment(), std::bool_constant<false>());
        }
      }
      if constexpr (M2Major == Eigen::RowMajor) {
        for (int i = 0; i < m1cols_m2rows; i++) {
          ...
          for (int j = 0; j < m2cols; j++) {
            this->matrix_func2.right_jacobian_domain_product(
                jx.template middleRows<M1Rows>(m1rows * j, m1rows), dvec,
                jxm2.row(m2cols * i + j), PlusEqualsAssignment(),
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian_impl(
    ...
    ///////////////////////////
    auto Impl = [&](auto& fxm1,auto & adjv1, auto& jxm1, auto& fxm2,auto & adjv2, auto& jxm2, auto& gxm2,
                    auto& hxm2, auto& fxmd, auto& dvec,auto & jttemp) {
      ...
      if constexpr (M1Major == Eigen::ColMajor) {
        ...
        for (int i = 0; i < m2cols; i++) {
          for (int j = 0; j < m1rows; j++) {
            adjv1.template segment<M1Cols_M2Rows>(m1cols_m2rows * j,
                                                  m1cols_m2rows) +=
      ...
      if constexpr (M2Major == Eigen::ColMajor) {
        for (int i = 0; i < m2cols; i++) {
          adjv2.template segment<M1Cols_M2Rows>(m1cols_m2rows * i,
                                                m1cols_m2rows) +=
              m1.transpose() *
              adjvars.template segment<M1Rows>(m1rows * i, m1rows);
        }
      } else {
        for (int i = 0; i < m1cols_m2rows; i++) {
          for (int j = 0; j < m2cols; j++) {
            adjv2[m2cols * i + j] += m1.col(i).dot(
                adjvars.template segment<M1Rows>(m1rows * j, m1rows));
          }
        }
      }
      ...
      this->matrix_func2.compute_jacobian_adjointgradient_adjointhessian(
          x, fxm2, jxm2, gxm2, hxm2, adjv2);

      this->matrix_func2.accumulate_hessian(adjhess, hxm2,
                                            PlusEqualsAssignment());
      this->matrix_func2.accumulate_gradient(adjgrad, gxm2,
                                             PlusEqualsAssignment());
      ...
      if constexpr (M2Major == Eigen::ColMajor) {
        if constexpr (M1Major == Eigen::ColMajor) {
          ...
          for (int i = 0; i < m2cols; i++) {
            for (int j = 0; j < m1rows; j++) {
              lvec.setConstant(
                  adjvars.template segment<M1Rows>(m1rows * i, m1rows)[j]);

              this->matrix_func1.right_jacobian_domain_product(
                  jttemp.template middleRows<M1Cols_M2Rows>(m1cols_m2rows * i,
                                                            m1cols_m2rows),
                  lvec.asDiagonal(),
                  jxm1.template middleRows<M1Cols_M2Rows>(m1cols_m2rows * j,
                                                          m1cols_m2rows),
        ...
        if constexpr (M1Major == Eigen::ColMajor ) {
            fxmd = adjvars;
            Eigen::Map<const Eigen::Matrix<Scalar, M2Cols, M1Rows, Eigen::RowMajor>> lmat(fxmd.data(),m2cols,m1rows);
      ...
      hxm2.setZero();
      this->matrix_func2.right_jacobian_product(hxm2, jttemp.transpose(), jxm2,
                                                DirectAssignment(),
                                                std::bool_constant<false>());
      ...
      if constexpr (MatFunc2::InputIsDynamic) {
        ...
        ASSET::constexpr_for_loop(
            ...
            std::integral_constant<int, sds>(), [&](auto i) {
              constexpr int Start1 =
                  MatFunc2::INPUT_DOMAIN::SubDomains[i.value][0];
              constexpr int Size1 =
                  MatFunc2::INPUT_DOMAIN::SubDomains[i.value][1];
