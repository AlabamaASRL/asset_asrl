...
struct TwoFunctionSum
    : TwoFunctionSum_Impl<TwoFunctionSum<Func1, Func2>, Func1, Func2, false> {
  using Base =
      TwoFunctionSum_Impl<TwoFunctionSum<Func1, Func2>, Func1, Func2, false>;
  using Base::Base;
...
struct FunctionDifference
    : TwoFunctionSum_Impl<FunctionDifference<Func1, Func2>, Func1, Func2,
                          true> {
  using Base =
...
struct MultiFunctionSum
    : MultiFunctionSum_Impl<MultiFunctionSum<Func1, Func2, Funcs...>, Func1,
                            Func2, Funcs...> {
  using Base = MultiFunctionSum_Impl<MultiFunctionSum<Func1, Func2, Funcs...>,
                                     Func1, Func2, Funcs...>;
  using Base::Base;
  DENSE_FUNCTION_BASE_TYPES(Base);
};
...
RetType make_dynamic_sum(const std::vector<FuncType>& funcs) {
  ...
  if (size == 0) {
    ...
    RetType summedT =
        make_sum(funcs[0], funcs[1], funcs[2], funcs[3], funcs[4]);
...
struct TwoFunctionSum_Impl
  ...
  static const bool func1_is_segment =
      Is_Segment<Func1>::value || Is_ScaledSegment<Func1>::value;
  static const bool func2_is_segment =
  ...
  static const bool IsSegmentOp =
      Is_Segment<Func1>::value && Is_Segment<Func2>::value;
  ...
  TwoFunctionSum_Impl() {}
  TwoFunctionSum_Impl(Func1 f1, Func2 f2)
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian_impl(
    ...
    //////////////////////////////////////////////
        ...
        auto Impl = [&](auto& func2_fx, auto& func2_jx, auto& func2_adjgrad,
            auto& func2_adjhess) {
  ...
  inline void right_jacobian_product(
    ...
    if constexpr (is_sum_of_segments) {
      ...
      if constexpr (std::is_same<Assignment, DirectAssignment>::value) {
        ...
        this->func2.right_jacobian_product(target_, left, right, assign,
                                           aliased);
      }
    } else if constexpr (func1_is_sumordiff && func2_is_segment) {
      if constexpr (Func1::is_sum_of_segments) {
        this->func1.right_jacobian_product(target_, left, right, assign,
                                           aliased);
        if constexpr (std::is_same<Assignment, DirectAssignment>::value) {
          this->func2.right_jacobian_product(target_, left, right,
                                             PlusEqualsAssignment(), aliased);
...
struct MultiFunctionSum_Impl
  ...
  static const bool IsSumofSegments =
      SZ_PROD<int(Is_Segment<Func1>::value || Is_ScaledSegment<Func1>::value),
              ...
              int(Is_Segment<Funcs>::value ||
                  Is_ScaledSegment<Funcs>::value)...>::value == 1;
  ...
  MultiFunctionSum_Impl(std::tuple<Func1, Func2, Funcs...> fs) {
    ...
    ASSET::constexpr_for_loop(
        ...
        std::integral_constant<int, sizeof...(Funcs)>(), [&](auto i) {
          std::get<i.value>(this->funcs) = std::get<i.value + 2>(fs);
        });
    ...
    ASSET::constexpr_for_loop(
        ...
        std::integral_constant<int, sizeof...(Funcs)>(), [&](auto i) {
          ...
          if (this->func1.ORows() != std::get<i.value>(this->funcs).ORows()) {
              fmt::print(fmt::fg(fmt::color::red),
                  "Math Error in MultiFunctionSum/+ method !!!\n"
                  "Output Size of Func1 (ORows = {0:}) does not match Output Size of Func{2:} (ORows = {1:}).\n",
                  this->func1.ORows(), std::get<i.value>(this->funcs).ORows(), i.value);
              throw std::invalid_argument("");
          }
          if (this->func1.IRows() != std::get<i.value>(this->funcs).IRows()) {
              fmt::print(fmt::fg(fmt::color::red),
                  "Math Error in MultiFunctionSum/+ method !!!\n"
                  "Input Size of Func1 (IRows = {0:}) does not match Input Size of Func{2:} (IRows = {1:}).\n",
                  this->func1.IRows(), std::get<i.value>(this->funcs).IRows(), i.value);
              throw std::invalid_argument("");
          }
          ...
          ASSET::constexpr_for_loop(
              ...
              std::integral_constant<int, sds>(), [&](auto i) {
                constexpr int Start1 =
                    FunciType::INPUT_DOMAIN::SubDomains[i.value][0];
                constexpr int Size1 =
                    FunciType::INPUT_DOMAIN::SubDomains[i.value][1];
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian_impl(
    ...
    auto Impl = [&](auto& func2_fx, auto& func2_jx, auto& func2_adjgrad,
      ...
      this->func2.accumulate_jacobian(jx, func2_jx, PlusEqualsAssignment());
      this->func2.accumulate_gradient(adjgrad_, func2_adjgrad,
                                      PlusEqualsAssignment());
      this->func2.accumulate_hessian(adjhess_, func2_adjhess,
                                     PlusEqualsAssignment());
      ...
      ASSET::tuple_for_each(this->funcs, [&](const auto& funci) {
        ...
        fx += func2_fx;
        funci.accumulate_jacobian(jx, func2_jx, PlusEqualsAssignment());
        funci.accumulate_gradient(adjgrad, func2_adjgrad,
                                  PlusEqualsAssignment());
        if constexpr (!FunciType::IsLinearFunction)
          funci.accumulate_hessian(adjhess, func2_adjhess,
                                   PlusEqualsAssignment());
      });
    };
    ...
    if constexpr (IsSumofSegments) {
      ...
      if constexpr (std::is_same<Assignment, DirectAssignment>::value) {
        ...
        ASSET::tuple_for_each(this->funcs, [&](const auto& func) {
          func.right_jacobian_product(target_, left, right,
                                      PlusEqualsAssignment(), aliased);
        });
      } else {
        this->func2.right_jacobian_product(target_, left, right, assign,
