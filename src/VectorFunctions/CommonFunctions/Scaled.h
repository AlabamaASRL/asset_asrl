...
struct StaticScaled
    : StaticScaled_Impl<StaticScaled<Func, Value>, Func, Value> {
...
struct StaticScaled_Impl
  ...
  inline void right_jacobian_product(
    ...
    this->func.right_jacobian_product(target_, left, right, assign,
                                                   aliased);
  }
  template <class Target, class Left, class Right, class Assignment,
            bool Aliased>
  inline void symetric_jacobian_product(
      ...
      ConstEigenBaseRef<Right> right, Assignment assign,
      std::bool_constant<Aliased> aliased) const {
    this->func.symetric_jacobian_product(target_, left, right,
                                                      assign, aliased);
  }
...
struct Scaled_Impl : VectorFunction<Derived, Func::IRC, Func::ORC, Analytic> {
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian_impl(
    ...
    MemoryManager::allocate_run(orows, Impl,
  ...
  inline void right_jacobian_product(
    ...
    if constexpr (Is_EigenDiagonalMatrix<Left>::value) {
      ...
      this->func.right_jacobian_product(target_, left_ref, right_ref,
                                               assign, aliased);
    } else {
      ConstMatrixBaseRef<Right> right_ref(right.derived());
  ...
  inline void symetric_jacobian_product(
      ...
      ConstEigenBaseRef<Right> right, Assignment assign,
      std::bool_constant<Aliased> aliased) const {
    this->func.symetric_jacobian_product(target_, left, right, assign,
                                                aliased);
  }
  template <class Target, class JacType, class Assignment>
  inline void accumulate_jacobian(ConstMatrixBaseRef<Target> target_,
...
struct RowScaled_Impl
  ...
  RowScaled_Impl(Func f, ConstVectorBaseRef<OutType> s)
      : func(std::move(f)) {
  ...
  inline void compute_impl(ConstVectorBaseRef<InType> x,
    ...
    ASSET::MemoryManager::allocate_run(orows, Impl,
        TempSpec<Output<Scalar>>(orows, 1)
    );
  ...
  inline void compute_jacobian_impl(ConstVectorBaseRef<InType> x,
    ...
    auto Impl = [&](auto& scales) {
        ...
        this->func.compute_jacobian(x, fx_, jx_);
        fx = fx.cwiseProduct(scales);
        this->func.right_jacobian_product(jx, scales.asDiagonal(), jx,
            DirectAssignment(),
...
template <class Derived, class Func,int MRows>
struct MatrixScaled_Impl
    : VectorFunction<Derived, Func::IRC, MRows, Analytic> {
    ...
    template <class OutType>
    MatrixScaled_Impl(Func f, ConstVectorBaseRef<OutType> s)
        : func(std::move(f)) {
        this->mat = s;
    ...
    inline void compute_jacobian_impl(ConstVectorBaseRef<InType> x,
        ...
        if (NoTemp) {
            auto Impl = [&](auto& mattmp) {
                ...
                mattmp = this->mat.template cast<Scalar>();
                fx = (mattmp * fx).eval();
                this->func.right_jacobian_product(jx, mattmp, jx,
                    DirectAssignment(),
                    std::bool_constant<true>());
            ...
            auto Impl = [&](auto& mattmp, auto& fxtmp,auto & jxtmp) {
                ...
                mattmp = this->mat.template cast<Scalar>();
                fx = (mattmp * fxtmp);
                this->func.right_jacobian_product(jx, mattmp, jxtmp,
                    DirectAssignment(),
