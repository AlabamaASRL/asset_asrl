...
struct VectorScalarFunctionProduct
    : VectorScalarFunctionProduct_Impl<
          VectorScalarFunctionProduct<VecFunc, ScalFunc>, VecFunc, ScalFunc> {
  using Base = VectorScalarFunctionProduct_Impl<
      VectorScalarFunctionProduct<VecFunc, ScalFunc>, VecFunc, ScalFunc>;
...
template <class Derived, class VecFunc, class ScalFunc>
struct VectorScalarFunctionProduct_Impl
    : VectorFunction<Derived, SZ_MAX<VecFunc::IRC, ScalFunc::IRC>::value,
                     VecFunc::ORC> {
  using Base =
      VectorFunction<Derived, SZ_MAX<VecFunc::IRC, ScalFunc::IRC>::value,
                     VecFunc::ORC>;
  ...
  using INPUT_DOMAIN =
      CompositeDomain<Base::IRC, typename VecFunc::INPUT_DOMAIN,
                      typename ScalFunc::INPUT_DOMAIN>;
  static const bool IsVectorizable =
      VecFunc::IsVectorizable && ScalFunc::IsVectorizable;
  ...
  VectorScalarFunctionProduct_Impl(VecFunc f1, ScalFunc f2)
    ...
    this->set_input_domain(
        this->IRows(), {scalarfunc.input_domain(), vectorfunc.input_domain()});

    if (this->scalarfunc.IRows() != this->vectorfunc.IRows()) {
        fmt::print(fmt::fg(fmt::color::red),
            "Math Error in VectorScalarFunctionProduct/ * method !!!\n"
            "Input Size of VectorFunc (IRows = {0:}) does not match Input Size of ScalarFunc (IRows = {1:}).\n",
  ...
  static const bool is_prod_of_segments =
      vectorfunc_is_segment && scalarfunc_is_segment;
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian_impl(
    ...
    auto Impl = [&](auto& jxs, auto& gxs, auto& hxs, auto& adjtemp) {
      ...
      this->scalarfunc.compute_jacobian_adjointgradient_adjointhessian(
          x, fxs, jxs, gxs, hxs, ls);
        ...
        if (sds == 0) {
          ...
          for (int i = 0; i < sds; i++) {
            ...
            adjhess.middleRows(start, size) +=
                hxs.middleCols(start, size).transpose();
          }
        }
      ...
      this->scalarfunc.accumulate_gradient(adjgrad, gxs,
