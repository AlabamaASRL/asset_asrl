...
true input domain that is computed at run-time. Also defines the default compile time INPUT_DOMAIN of both constant and dynamic size functions. 
Composite derived classes will overide this typedef by design, in order to explicitly participate in the input domain tracking system. 
The default input domain simply says that all inputs are used. This input domain info is used to implement the default set of functions for
manipluting the jacobian,hessian,and gradient of Derived function such as right_jacobian_product. Specialized derived classes should perform
simple overloads of these methods.
...
Additionally, This class also defines the .compute_jacobian etc. methods in terms of the compute_jacobian_impl methods implemented
in derived classes. Theses methods are the used to implement the dense vector functions interface with psiopt throught the 
constraints_jacobian etc. methods.

Finally, this class also contains methods (Postfixed Build) defining the python binding code of a vector functions public interface.
...
struct DenseFunctionBase : Computable<Derived, IR, OR>, DomainHolder<IR> {
  ...
  using SEGMENTOP =
      typename ASSET::template NestedFunctionSelector<Segment<OR, SZ, ST>,
      Derived>;
  ...
  template <int SZ, int ST>
  auto segment() const {
    return SEGMENTOP<SZ, ST>::make_nested(
        Segment<OR, SZ, ST>(this->ORows(), SZ, ST), this->derived());
  }

  template <int SZ, int ST>
  auto segment(int start, int size) const {
    return SEGMENTOP<SZ, ST>::make_nested(
        Segment<OR, SZ, ST>(this->ORows(), size, start), this->derived());
  }

  auto segment(int start, int size) const {
    return SEGMENTOP<-1, -1>::make_nested(
        Segment<OR, -1, -1>(this->ORows(), size, start), this->derived());
  }

  template <int SZ>
  auto segment(int start) const {
    return SEGMENTOP<SZ, -1>::make_nested(
        Segment<OR, SZ, -1>(this->ORows(), SZ, start), this->derived());
  }
  template <int SZ>
  auto head() const {
    return SEGMENTOP<SZ, 0>::make_nested(
        Segment<OR, SZ, 0>(this->ORows(), SZ, 0), this->derived());
  }
  template <int SZ>
  ...
  auto head(int sz) const {
      return SEGMENTOP<-1, -1>::make_nested(
          Segment<OR, -1, -1>(this->ORows(), sz, 0), this->derived());
  }

  template <int SZ>
  auto tail() const {
    return SEGMENTOP<SZ, SZ_DIFF<OR, SZ>::value>::make_nested(
        Segment<OR, SZ, SZ_DIFF<OR, SZ>::value>(this->ORows(), SZ,
                                                this->ORows() - SZ),
        this->derived());
  }
  template <int SZ>
  ...
  decltype(auto) coeff() const {
    return SEGMENTOP<1, ELE>::make_nested(
        Segment<OR, 1, ELE>(this->ORows(), 1, ELE), this->derived());
  }
  template <int ELE>
  ...
  auto coeff(int ele) const {
    return SEGMENTOP<1, -1>::make_nested(
        Segment<OR, 1, -1>(this->ORows(), 1, ele), this->derived());
  }
  ...
  decltype(auto) normalized() const {
    return FWDOP<Normalized<OR>>::make_nested(Normalized<OR>(this->ORows()),
                                              this->derived());
  }
  template <int PW>
  decltype(auto) normalized_power() const {
    return FWDOP<NormalizedPower<OR, PW>>::make_nested(
        NormalizedPower<OR, PW>(this->ORows()), this->derived());
  }

  decltype(auto) norm() const {
    return FWDOP<Norm<OR>>::make_nested(Norm<OR>(this->ORows()),
                                        this->derived());
  }
  decltype(auto) squared_norm() const {
    return FWDOP<SquaredNorm<OR>>::make_nested(SquaredNorm<OR>(this->ORows()),
                                               this->derived());
  }
  decltype(auto) inverse_norm() const {
    return FWDOP<InverseNorm<OR>>::make_nested(InverseNorm<OR>(this->ORows()),
                                               this->derived());
  }
  decltype(auto) inverse_squared_norm() const {
    return FWDOP<InverseSquaredNorm<OR>>::make_nested(
        InverseSquaredNorm<OR>(this->ORows()), this->derived());
  }
  template <int PW>
  decltype(auto) norm_power() const {
    return FWDOP<NormPower<OR, PW>>::make_nested(
        NormPower<OR, PW>(this->ORows()), this->derived());
  }
  template <int PW>
  decltype(auto) inverse_norm_power() const {
    return FWDOP<InverseNormPower<OR, PW>>::make_nested(
        InverseNormPower<OR, PW>(this->ORows()), this->derived());
  }
  /////////////////////////////////////////////////////////////////////////////////////////////////////////

  template <int MRows, int MCols, int MMajor = Eigen::ColMajor>
  auto matrix(int rows, int cols) const {
    return MatrixFunctionView<Derived, MRows, MCols, MMajor>(this->derived(),
                                                             rows, cols);
  }

  auto colmatrix(int rows, int cols) const {
      return MatrixFunctionView<Derived, -1, -1, Eigen::ColMajor>(this->derived(),
          rows, cols);
  }
  auto rowmatrix(int rows, int cols) const {
      return MatrixFunctionView<Derived, -1, -1, Eigen::RowMajor>(this->derived(),
          rows, cols);
  }
  ...
  auto colvector() const {
    return MatrixFunctionView<Derived, OR, 1, Eigen::ColMajor>(
        this->derived(), this->ORows(), 1);
  }
  auto rowvector() const {
    return MatrixFunctionView<Derived, 1, OR, Eigen::ColMajor>(
        this->derived(), 1, this->ORows());
  }
  ...
  decltype(auto) sum() const {
      if constexpr (OR == 1) return Derived(this->derived());
      else return CwiseSum<Derived>(this->derived());
  }
  ...
  decltype(auto) scale(const DenseFunctionBase<ScalFunc, IR, 1>& f) const {
    return VectorScalarFunctionProduct<Derived, ScalFunc>(this->derived(),
                                                          f.derived());
  }
  ...
  inline void compute_jacobian(ConstVectorBaseRef<InType> x,
    ...
    if constexpr (!Derived::IsVectorizable) {
      if constexpr (Is_SuperScalar<Scalar>::value) {
        ...
        if constexpr (Base::InputIsDynamic) x_r.resize(IRR);
        if constexpr (Base::OutputIsDynamic) fx_r.resize(ORR);
        if constexpr (Base::JacobianIsDynamic) jx_r.resize(ORR, IRR);

        for (int i = 0; i < Scalar::SizeAtCompileTime; i++) {
          for (int j = 0; j < IRR; j++) x_r[j] = x[j][i];
          this->derived().compute_jacobian_impl(x_r, fx_r, jx_r);
          for (int j = 0; j < ORR; j++) fx[j][i] = fx_r[j];

          for (int j = 0; j < IRR; j++)
            for (int k = 0; k < ORR; k++) jx(k, j)[i] = jx_r(k, j);
  ...
  inline void jacobian(ConstVectorBaseRef<InType> x,
                       ConstMatrixBaseRef<JacType> jx_) const {
    typedef typename InType::Scalar Scalar;
  ...
  inline Jacobian<typename InType::Scalar> jacobian(
      ConstVectorBaseRef<InType> x) const {
    typedef typename InType::Scalar Scalar;
  ...
  inline void compute_adjointgradient(
  ...
  inline void compute_jacobian_adjointgradient_adjointhessian(
    ...
    if constexpr (!Derived::IsVectorizable) {
      ...
      if constexpr (Is_SuperScalar<Scalar>::value) {
        ...
        if constexpr (Base::InputIsDynamic) x_r.resize(IRR);
        if constexpr (Base::InputIsDynamic) gx_r.resize(IRR);
        if constexpr (Base::InputIsDynamic) hx_r.resize(IRR, IRR);

        if constexpr (Base::OutputIsDynamic) fx_r.resize(ORR);
        if constexpr (Base::OutputIsDynamic) l_r.resize(ORR);

        if constexpr (Base::JacobianIsDynamic) jx_r.resize(ORR, IRR);

        for (int i = 0; i < Scalar::SizeAtCompileTime; i++) {
          for (int j = 0; j < IRR; j++) x_r[j] = x[j][i];
          for (int j = 0; j < ORR; j++) l_r[j] = adjvars[j][i];

          this->derived().compute_jacobian_adjointgradient_adjointhessian_impl(
              x_r, fx_r, jx_r, gx_r, hx_r, l_r);
          for (int j = 0; j < ORR; j++) fx[j][i] = fx_r[j];
          for (int j = 0; j < IRR; j++) adjgrad[j][i] = gx_r[j];

          for (int j = 0; j < IRR; j++)
            for (int k = 0; k < ORR; k++) jx(k, j)[i] = jx_r(k, j);

          for (int j = 0; j < IRR; j++)
            for (int k = 0; k < IRR; k++) adjhess(k, j)[i] = hx_r(k, j);
  ...
  inline void adjointhessian(ConstVectorBaseRef<InType> x,
    ...
    this->derived().compute_jacobian_adjointgradient_adjointhessian(
        x, fx, jx, adjgrad, adjhess_, adjvars);
  }

  template <class InType, class AdjVarType>
  inline Hessian<typename InType::Scalar> adjointhessian(
  ...
  /*
    ...
    us to avoid multupllying explicit blocks of zeros that may be present in right, and can be computed from the
    input domain of derived. The default implementation here only detects non-zero columns as deternmiend from the input domain.
    The actual implementaion of the products are in DenseFunctionOperations.h, and the one called depends
    on whether the input domain of Derived is known at compile time or not. Derived objects can overload this function in order to
    implement a product that takes advantage of the exact non-zero pattern of the jacobian.
  */
  ...
  inline void right_jacobian_product(
    ...
    if constexpr (Base::InputIsDynamic 
      ...
      if constexpr (Base::InputIsDynamic) {
        ...
        ASSET::right_jacobian_product_impl(target_, left, right, assign,
                                           aliased);
      }
      ...
      ASSET::right_jacobian_product_constant_impl(DMN(), target_, left, right,
                                                  assign, aliased);
    }
  }
  ...
  /*
    ...
    input domain of derived. The default implementation here only detects non-zero columns as deternmiend from the input domain.
    The actual implementaion of the products are in DenseFunctionOperations.h, and the one called depends
    on whether the input domain of Derived is known at compile time or not. Derived objects should not overload this function.
  ...
  */
  ...
  inline void symetric_jacobian_product(
      ...
      std::bool_constant<Aliased> aliased) const {
    if constexpr (Base::InputIsDynamic ||
                  std::is_same<typename Derived::INPUT_DOMAIN,
                               INPUT_DOMAIN>::value) {
      if constexpr (Base::InputIsDynamic) {
        ASSET::symetric_jacobian_product_dynamic_impl(
            this->SubDomains, target_, left, right, assign, aliased);
      } else {
        ASSET::symetric_jacobian_product_impl(target_, left, right, assign,
                                              aliased);
      }

    } else {
      using DMN = typename Derived::INPUT_DOMAIN;
      ASSET::symetric_jacobian_product_constant_impl(DMN(), target_, left,
  ...
  inline void accumulate_jacobian(ConstMatrixBaseRef<Target> target_,
    ...
    if constexpr (Base::InputIsDynamic ||
      ...
      ASSET::constexpr_for_loop(
          ...
          [&](auto i) {
            constexpr int start =
                Derived::INPUT_DOMAIN::SubDomains[i.value][0];
    ...
    if constexpr (Base::InputIsDynamic ||
      ...
      ASSET::constexpr_for_loop(
          ...
          [&](auto i) {
            ...
            constexpr int size = Derived::INPUT_DOMAIN::SubDomains[i.value][1];
            ASSET::accumulate_impl(
                target_ref.template segment<size>(start, size),
  ...
  inline void accumulate_hessian(ConstMatrixBaseRef<Target> target_,
    ...
    if constexpr (Derived::IsLinearFunction) {
      ...
      if constexpr (sds == 1) {
        ...
        ASSET::accumulate_impl(target_ref.template block<size, size>(
                                   start, start, size, size),
                               right_ref.template block<size, size>(
                                   start, start, size, size),
                               assign);
        ...
        ASSET::accumulate_impl(target_ref.template block<Size1, Size1>(
                                   Start1, Start1, Size1, Size1),
                               right_ref.template block<Size1, Size1>(
                                   Start1, Start1, Size1, Size1),
                               assign);

        ASSET::accumulate_impl(target_ref.template block<Size2, Size2>(
                                   Start2, Start2, Size2, Size2),
                               right_ref.template block<Size2, Size2>(
                                   Start2, Start2, Size2, Size2),
                               assign);

        ASSET::accumulate_impl(target_ref.template block<Size1, Size2>(
                                   Start1, Start2, Size1, Size2),
                               right_ref.template block<Size1, Size2>(
                                   Start1, Start2, Size1, Size2),
                               assign);

        ASSET::accumulate_impl(target_ref.template block<Size2, Size1>(
                                   Start2, Start1, Size2, Size1),
                               right_ref.template block<Size2, Size1>(
                                   Start2, Start1, Size2, Size1),
                               assign);
        ...
        ASSET::constexpr_for_loop(
            ...
            std::integral_constant<int, sds>(), [&](auto i) {
              constexpr int start =
                  Derived::INPUT_DOMAIN::SubDomains[i.value][0];
              constexpr int size =
                  Derived::INPUT_DOMAIN::SubDomains[i.value][1];
              ASSET::accumulate_impl(
                  target_ref.template middleCols<size>(start, size),
                  right_ref.template middleCols<size>(start, size), assign);
            });
      ...
      if constexpr (Derived::InputIsDynamic) {
          ...
          if (sds == 0) {
              ...
              for (int i = 0; i < sds; i++) {
                  ...
                  target_ref.middleRows(start, size) +=
                      right.middleCols(start, size).transpose();
              }
          }
      }
      else {
    ...
    if constexpr (Base::InputIsDynamic ||
      ...
      ASSET::constexpr_for_loop(
          ...
          [&](auto i) {
            ...
            constexpr int size = Derived::INPUT_DOMAIN::SubDomains[i.value][1];
            this->scale_impl(
                target_ref.template middleCols<size>(start, size), s);
          });
    }
  }
  template <class Target, class Scalar>
      ...
      ASSET::constexpr_for_loop(
          ...
          [&](auto i) {
            constexpr int start =
                Derived::INPUT_DOMAIN::SubDomains[i.value][0];
            constexpr int size = Derived::INPUT_DOMAIN::SubDomains[i.value][1];
            this->scale_impl(target_ref.template segment<size>(start, size),
                             s);
          });
    }
  }
  ...
  */
  ...
  void RunPrintTime(ConstEigenRef<Input<double>> xt, volatile int n,
    ...
    std::cout << "Compute_jacobian Time:" << tfun2.total() * 1000.0 << " ms"
              << std::endl;
    std::cout << "Dummy Value: " << t << std::endl;
    ...
    if constexpr (DoHessian) {
      ...
      std::cout << "Compute_jacobian_hessian Time:" << tfun3.total() * 1000.0
                << " ms" << std::endl;
      std::cout << "Dummy Value: " << t << std::endl;
    }
  }
  ...
  * they are not. However, this method will account for explicit non-zeros if the HessianElemIsNonZero ex. is
  * explicity overloaded by derived. The sum of the non-zeros here is used when interfacing whith the optimizer throutght the
  * wrapper classes ConstraintFunction and ObjectiveFunction.
  */
    ...
    for (int i = 0; i < this->IRows(); i++) {
      if (dohess) {
        for (int j = i; j < this->IRows(); j++) {
          if (this->derived().HessianElemIsNonZero(j, i)) hesselems++;
        }
      }
      if (dojac) {
        for (int j = 0; j < this->ORows(); j++) {
          if (this->derived().JacobianElemIsNonZero(j, i)) jacelems++;
        }
      }
    }
  ...
  * of derived inside of the KKT matrix of an optimization problem when it is used as a constraint. Meta data
  * containing the locations of the input variables and constraint row for each call of derived are held in the 
  * Solver indexing data struct.
  */
  ...
  * algorithm. Vector X is the total variables vector for the full optimization problem, and FX is the total eqaility 
    or inequality constraints vector for the problem. Sparse Matrix KKTMat is the full KKT matrix of the problem. 
    KKTLocations contains the location where every non-zero element of each jacobian arrising from of a call to derived should be summed
    into KKTMat. KKTClashes contains a flag (-1) indicating if we can sum into the corresponding column without clashing with the work of
    another function on another thread. If the flag is greater than zero, the element of KKTClash is the index of the mutex in KKTLocks,
    which derived should lock when summing into the column and then release. This logic is controled by KKTFillAll and KKTFillJac.
  ...
  */
  ...
  /*
  * This function is the interface allowing Derived to be used as a constraint inside psiopt.
  * It computes both the value of the function and its jacobain gradient and hessian and is called during psiopts optimize
  * algorithm. Vector X is the total variables vector for the full optimization problem, and FX is the total eqaility
    or inequality constraints vector for the problem. Vector L is the vector of  equality or inequlity lagrange multipliers
    for the full optimization problem and.
    Sparse Matrix KKTMat is the full KKT matrix of the problem.
    KKTLocations contains the location where every non-zero element of each jacobian arrising from of call to derived should be summed
    into KKTMat. KKTClashes contains a flag (-1) indicating if we can sum into the corresponding column without clashing with the work of
    another function on another thread. If the flag is greater than zero, the element of KKTClash is the index of the mutex in KKTLocks,
    which derived should lock when summing into the column and then release. This logic is controled by KKTFillAll and KKTFillJac.
    Do not overload these functions unless you really know what you are doing.
    ...
    auto VectorImpl = [&]() {
      ...
      for (int i = 0; i < Packs; i++) {
        ...
        for (int j = 0; j < vsize; j++) {
          ...
          this->derived().KKTFillAll(V, jx, hx, KKTmat, KKTLocations,
 ...
 protected:
  ...
  inline void AddHessianElem(double v, int row, int col, double* mpt,
                             const int* lpt, int& freeloc) const {
    if constexpr (!Derived::IsLinearFunction) {
      mpt[lpt[freeloc]] += v;
      freeloc++;
    }
  }
  inline void AddJacobianElem(double v, int row, int col, double* mpt,
                              const int* lpt, int& freeloc) const {
    mpt[lpt[freeloc]] += v;
    freeloc++;
  }

  void KKTFillAll(int Apl, const Jacobian<double>& jx,
  ...
  void KKTFillJac(int Apl, const Jacobian<double>& jx,
    ...
    if (data.unique_constraints) {
      for (int i = 0; i < this->IRows(); i++) {
        ActiveVar = data.VLoc(i, Apl);
        for (int j = i; j < this->IRows(); j++) {
          if (this->derived().HessianElemIsNonZero(j, i)) freeloc++;
        }
        for (int j = 0; j < this->ORows(); j++) {
          this->derived().AddJacobianElem(jx(j, i), j, i, mpt, lpt, freeloc);
        }
  ...
  */
  ...
  static void DenseBaseBuild(PYClass& obj) {
      ...
      obj.def("compute", [](const Derived& func, ConstEigenRef<Input<double>> x) {
          if (x.size() != func.IRows())throw std::invalid_argument("Incorrectly sized input to function");
      ...
      obj.def("jacobian",
          [](const Derived& func, ConstEigenRef<Input<double>> x) {
              if (x.size() != func.IRows())throw std::invalid_argument("Incorrectly sized input to function");
      ...
      obj.def("adjointgradient",
          [](const Derived& func, ConstEigenRef<Input<double>> x,
              ConstEigenRef<Output<double>> lm) {
                  if (x.size() != func.IRows())throw std::invalid_argument("Incorrectly sized input to function");
                  if (lm.size() != func.ORows())throw std::invalid_argument("Incorrectly sized multiplier input to function");
  ...
  static void DoubleMathBuild(PYClass& obj) {
      ...
      if constexpr (OR == 1) { // Scalars
          ...
          obj.def(
              "__rmul__", [](const Derived& a, const Eigen::VectorXd& b) {
                  return Gen(MatrixScaled<Derived, -1>(a, b));
              },
              py::is_operator());
          
          obj.def(
              "__mul__", [](const Derived& a, const Eigen::VectorXd & b) { 
                  return Gen(MatrixScaled<Derived,-1>(a,b)); 
              },
              py::is_operator());
      ...
      if constexpr (OR != 1) {
          ...
          if constexpr (OR > 0) { // already constant size
              ...
              auto SizeSwitch = [](const auto & fun, auto Lam) {
                  ...
                  //else
                  if (orr == 3)return Lam(fun, std::integral_constant<int, 3>());
                  return Lam(fun, std::integral_constant<int, -1>());
              };

              obj.def(
                  "norm",
                  [SizeSwitch](const Derived& fun) {
  ...
  static void FunctionIndexingBuild(PYClass& obj) {
      ...
      obj.def("padded_lower",
          [](const Derived& a, int lpad) {
              return Gen(a.padded_lower(lpad));
          });
      obj.def("padded_upper",
          [](const Derived& a, int upad) {
              return Gen(a.padded_upper(upad));
          });
      obj.def("padded",
          [](const Derived& a, int upad, int lpad) {
              return Gen(a.padded(upad, lpad));
          });
      ...
      obj.def("head",[](const Derived& a, int size) {
          return SegRetType(a.head(size));
          });
      obj.def("tail", [](const Derived& a, int size) {
          return SegRetType(a.tail(size));
          });

      obj.def("coeff", [](const Derived& a, int elem) {
          return ElemRetType(a.coeff(elem)); });
      obj.def(
      ...
      if constexpr (OR < 0 || OR > 2) {
          ...
          auto head2 = [](const Derived& a) {
              return Seg2RetType(a.template segment<2>(0));
          };
      ...
      if constexpr (OR < 0 || OR > 3) {
          ...
          auto head3 = [](const Derived& a) {
              return Seg3RetType(a.template segment<3>(0));
          };
  ...
  static void BinaryMathBuild(PYClass& obj) {
      ...
      if constexpr (OR == 3 || OR == -1) {
          ...
          if constexpr (!is_seg3)
              obj.def(
                  "cross",
                  [](const Derived& seg1, const SEG3& seg2) {
                      return Gen(crossProduct(seg1, seg2));
                  });
          if constexpr (!is_gen)
              obj.def(
                  "cross",
                  [](const Derived& seg1, const Gen& seg2) {
                      return Gen(crossProduct(seg1, seg2));
                  });

          obj.def(
              "cross",
              [](const Derived& seg1, const Vector3<double> & seg2) {
          ...
          obj.def(
              "cross",
              [](const Derived& seg1, const Derived& seg2) {
                  return Gen(crossProduct(seg1, seg2));
              });
          ...
          obj.def(
              "dot",
              [](const Derived& seg1, const Output<double>& seg2) {
  ...
  static void BinaryOperatorsBuild(PYClass& obj) {
      ...
      if constexpr(!is_arglike) {
          ...
          obj.def(
              "eval",
              [](const Derived& a, const ELEM & b) {
                  return BinGen(a.eval(b));
              });
          obj.def(
              "eval",
              [](const Derived& a, const SEG& b) {
                  return BinGen(a.eval(b));
              });
          obj.def(
              "eval",
              [](const Derived& a, const SEG2& b) {
                  return BinGen(a.eval(b));
              });
          obj.def(
              "eval",
              [](const Derived& a, const SEG3& b) {
                  return BinGen(a.eval(b));
              });
          obj.def(
              "eval",
          ...
          obj.def(
              "eval",
              [](const Derived & a, int ir, Eigen::VectorXi v) {
  ...
  static void ConditionalOperatorsBuild(PYClass& obj) {
      ...
      if constexpr (OR == 1) {
          ...
          obj.def(
              "__le__",
              [](const Derived& a, double b) { return GenCon(a <= b); },
              py::is_operator());
          obj.def(
              "__ge__",
              [](const Derived& a, double b) { return GenCon(a >= b); },
          ...
          obj.def(
              "__le__",
              [](const Derived& a, const Derived& b) { return GenCon(a <= b); },
              py::is_operator());
          obj.def(
              "__ge__",
              [](const Derived& a, const Derived& b) { return GenCon(a >= b); },
