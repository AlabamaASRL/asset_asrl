...
Also defines implements the constraints function in terms of the compute function. The constraints functions is part of
a vector functions interface to the non-linear optimzier PSIOPT.
...
struct ComputableBase : CRTPBase<Derived>, InputOutputSize<IR, OR> {
  ...
  inline void compute(ConstVectorBaseRef<InType> x,
    ...
    if constexpr (!Derived::IsVectorizable) {
      if constexpr (Is_SuperScalar<Scalar>::value) {
        ...
        if constexpr (InputIsDynamic) x_r.resize(IRR);
        if constexpr (OutputIsDynamic) fx_r.resize(ORR);
  ...
  inline Output<typename InType::Scalar> compute(
  ...
  inline Gradient<typename InType::Scalar> adjointgradient(
  ...
  * Vector X is the total variables vector for the full optimization problem, and FX is the total eqaility 
    or inequality constraints vector for the problem. Data pertaining to the loction of the input variables in X
    for each distinct call as well as the target location for the output variables in FX is stored in the SolverIndexingData
    object. In general users should not directly overload this function unless they have a very good reason.
  */
  ...
  */
  inline void gatherInput(ConstEigenRef<Eigen::VectorXd> X, Input<double> &xt,
                          int V, const SolverIndexingData &data) const {
    if (data.VindexContinuity[V] == ParsedIOFlags::Contiguous) {
      xt = X.template segment<IR>(data.VLoc(0, V), this->IRows());
    } else {
      for (int i = 0; i < this->IRows(); i++) xt(i) = X(data.VLoc(i, V));
    }
  }
  ...
  */
  inline void gatherMult(ConstEigenRef<Eigen::VectorXd> L, Output<double> &l,
                         int V, const SolverIndexingData &data) const {
    if (data.CindexContinuity[V] == ParsedIOFlags::Contiguous) {
      l = L.template segment<OR>(data.CLoc(0, V), this->ORows());
    } else {
      for (int i = 0; i < this->ORows(); i++) l(i) = L(data.CLoc(i, V));
    }
  }
};
