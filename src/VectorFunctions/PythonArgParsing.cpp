    ...
    std::vector<GenericFunction<-1, -1>> ParsePythonArgs(py::args x, int irows) {
        ...
        for (auto xi = x.begin(); xi != x.end(); ++xi)
        {
            ...
            if (xi->get_type().is(vftype) ||
                xi->get_type().is(sftype) ||
                xi->get_type().is(elemtype) ||
                xi->get_type().is(segtype) ||
                xi->get_type().is(seg2type) ||
                xi->get_type().is(seg3type) ||
            ...
            else if (xi->get_type().is(py_float) ||
                xi->get_type().is(py_int) ||
                xi->get_type().is(np_int) ||
                xi->get_type().is(np_float)) {

                // Good to go
            }
            else if (xi->get_type().is(py_list) ||
                xi->get_type().is(np_array)) {
