...
for an asset vector function to be used as a constraint or objective inside of psiopt. It is coupled with a function
by the interface classes ConstraintFunction and ObjectiveFunction.
...
struct SolverIndexingData {
  ...
  SolverIndexingData() {}
  SolverIndexingData(int irr, int orr, const MatrixXi& vindex,
                     const MatrixXi& cindex)
      : input_size(irr), output_size(orr) {
    this->setVindexCindex(vindex, cindex);
  }

  SolverIndexingData(int irr, const MatrixXi& vindex)
      : input_size(irr), output_size(1) {
  ...
  std::vector<SolverIndexingData> thread_split(int Threads) const {
    ...
    for (int i = 0; i < range; i++) {
      if (this->cindex_init) {
        split.emplace_back(
            SolverIndexingData(this->input_size, this->output_size,
                               this->Vindex.middleCols(start, perThr[i]),
                               this->Cindex.middleCols(start, perThr[i])));
      } else {
        split.emplace_back(SolverIndexingData(
            this->input_size, this->Vindex.middleCols(start, perThr[i])));
      }
  ...
  static ParsedIOFlags checkContinuity(const Eigen::VectorXi& ix) {
    ...
    for (int i = 0; i < (ix.size() - 1); i++) {
      s = ix[i + 1] - ix[i] - 1;
      if (s != 0) return ParsedIOFlags::NotContiguous;
    }
