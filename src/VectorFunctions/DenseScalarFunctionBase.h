...
struct DenseScalarFunctionBase : DenseFunctionBase<Derived, IR, 1> {
  ...
  void objective_gradient(double ObjScale, ConstEigenRef<Eigen::VectorXd> X,
                          double& Val, EigenRef<Eigen::VectorXd> GX,
                          const SolverIndexingData& data) const {
    Input<double> x(this->IRows());
    Output<double> fx(1);
    ...
    for (int V = 0; V < data.NumAppl(); V++) {
      this->gatherInput(X, x, V, data);
      new (&gx) Eigen::Map<Input<double>>(
          GX.data() + data.InnerGradientStarts[V], this->IRows());
      fx.setZero();
      jx.setZero();
      gx.setZero();
  ...
  void objective_gradient_hessian(
    ...
    for (int V = 0; V < data.NumAppl(); V++) {
      ...
      Val += fx[0] * ObjScale;
      this->KKTFillHess(V, hx, KKTmat, KKTLocations, KKTClashes, KKTLocks,
                        data);
    }
  }
  //////////////////////////////////////////////////////////////////////////////
