  ...
  if constexpr (std::is_same<Assignment, DirectAssignment>::value) {
    ...
    if constexpr (Aliased) {
      ...
      target.noalias() -= left.derived() * right.derived();
                                    ...
                                    ScaledDirectAssignment<Scalar>>::value) {
    if constexpr (Aliased) {
      target = assign.value * left.derived() * right.derived();
    } else {
      target.noalias() = assign.value * left.derived() * right.derived();
    }
  } else if constexpr (std::is_same<Assignment, ScaledPlusEqualsAssignment<
                                                    Scalar>>::value) {
    if constexpr (Aliased) {
      target += assign.value * left.derived() * right.derived();
    } else {
...
template <class INPUT_DOMAIN, class Target, class Left, class Right,
          class Assignment, bool Aliased>
void right_jacobian_product_constant_impl(
    const INPUT_DOMAIN& SubDomains, const Eigen::MatrixBase<Target>& target_,
    const Eigen::EigenBase<Left>& left, const Eigen::EigenBase<Right>& right,
    Assignment assign, std::bool_constant<Aliased> aliased) {
...
template <class Target, class Left, class Right, class Assignment, bool Aliased>
void right_jacobian_product_dynamic_impl(
    const DomainMatrix& SubDomains, const Eigen::MatrixBase<Target>& target_,
    const Eigen::EigenBase<Left>& left, const Eigen::EigenBase<Right>& right,
    Assignment assign, std::bool_constant<Aliased> aliased) {
  ...
  if (sds == 0) {
    ...
    for (int i = 0; i < sds; i++) {
      ...
      ASSET::right_jacobian_product_impl(
          target_ref.middleCols(start, size), left,
...
void symetric_jacobian_product_impl(const Eigen::MatrixBase<Target>& target_,
  ...
  if constexpr (std::is_same<Assignment, DirectAssignment>::value) {
    if constexpr (Aliased) {
      ...
      target.noalias() =
          right.derived().transpose() * left.derived() * right.derived();
    }
  } else if constexpr (std::is_same<Assignment, PlusEqualsAssignment>::value) {
    if constexpr (Aliased) {
      target += right.derived().transpose() * left.derived() * right.derived();
    } else {
      if constexpr (Left::MaxRowsAtCompileTime == 1 &&
                    Left::MaxColsAtCompileTime==1) {
        target.noalias() += right.derived().transpose() *
                            (Scalar(left.derived()[0]) * right.derived());
    ...
    if constexpr (Aliased) {
      ...
      target.noalias() -=
          right.derived().transpose() * left.derived() * right.derived();
                                    ...
                                    ScaledDirectAssignment<Scalar>>::value) {
    if constexpr (Aliased) {
      target = assign.value * right.derived().transpose() * left.derived() *
               right.derived();
    } else {
      target.noalias() = assign.value * right.derived().transpose() *
...
template <class INPUT_DOMAIN, class Target, class Left, class Right,
          class Assignment, bool Aliased>
void symetric_jacobian_product_constant_impl(
    const INPUT_DOMAIN& SubDomains, const Eigen::MatrixBase<Target>& target_,
    const Eigen::EigenBase<Left>& left, const Eigen::EigenBase<Right>& right,
    Assignment assign, std::bool_constant<Aliased> aliased) {
  constexpr int sds = INPUT_DOMAIN::SubDomains.size();
    ...
    ASSET::constexpr_for_loop(
        ...
        [&](auto i) {
          ...
          ASSET::symetric_jacobian_product_impl(
              target_ref.template block<Size1, Size1>(Start1, Start1, Size1,
                                                      Size1),
              left, right_ref.template middleCols<Size1>(Start1, Size1), assign,
              aliased);
        });
  }
}
...
template <class Target, class Left, class Right, class Assignment, bool Aliased>
void symetric_jacobian_product_dynamic_impl(
    const DomainMatrix& SubDomains, const Eigen::MatrixBase<Target>& target_,
    const Eigen::EigenBase<Left>& left, const Eigen::EigenBase<Right>& right,
    Assignment assign, std::bool_constant<Aliased> aliased) {
  const int sds = SubDomains.cols();
  ...
  if (sds == 0 || sds > 1) {
    ...
    for (int i = 0; i < sds; i++) {
      ...
      ASSET::symetric_jacobian_product_impl(
          target_ref.block(start, start, size, size), left,
          right_ref.middleCols(start, size), assign, aliased);
...
template <class Target, class JacType, class Assignment>
void accumulate_matrix_dynamic_domain_impl(
    const DomainMatrix& SubDomains, const Eigen::MatrixBase<Target>& target_,
  ...
  if (sds == 0) {
    ...
    for (int i = 0; i < sds; i++) {
      ...
      ASSET::accumulate_impl(target_ref.middleCols(start, size),
...
template <class Target, class JacType, class Assignment>
void accumulate_symetric_matrix_dynamic_domain_impl(
    const DomainMatrix& SubDomains, const Eigen::MatrixBase<Target>& target_,
    const Eigen::MatrixBase<JacType>& right, Assignment assign) {
  int sds = SubDomains.cols();
  const Eigen::MatrixBase<JacType>& right_ref(right.derived());
...
template <class Target, class JacType, class Assignment>
void accumulate_vector_dynamic_domain_impl(
    const DomainMatrix& SubDomains, const Eigen::MatrixBase<Target>& target_,
    const Eigen::MatrixBase<JacType>& right, Assignment assign) {
  int sds = SubDomains.cols();
  if (sds == 0) {
    ...
    for (int i = 0; i < sds; i++) {
      int start = SubDomains(0, i);
      int size = SubDomains(1, i);
      ASSET::accumulate_impl(target_ref.segment(start, size),
                             right_ref.segment(start, size), assign);
    }
  }
}
