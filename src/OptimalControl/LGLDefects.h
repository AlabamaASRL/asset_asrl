...
namespace ASSET {
    ...
    struct LGLDefects
            ...
            auto Impl = [&](auto& C_XS, auto& C_DXS, auto& I_XS, auto& I_DXS) {
                ...
                for (int i = 0; i < Cardinals; i++) {
                    C_XS[i].template segment<DODE::XtUV>(0, this->ode.XtUVars()) =
                        x.template segment<DODE::XtUV>(i * this->ode.XtUVars(),
                            this->ode.XtUVars());
                ...
                for (int i = 0; i < Interiors; i++) {
                    ...
                    for (int j = 0; j < Cardinals; j++) {
                        ...
                        I_XS[i].template segment<DODE::UV>(this->ode.XtVars(),
                            this->ode.UVars()) +=
                            Scalar(Coeffs::Cardinal_UPoly_Weights[i][j]) *
                            C_XS[j].template segment<DODE::UV>(this->ode.XtVars(),
                                this->ode.UVars());

                        fx.template segment<DODE::XV>(i * this->ode.XVars(),
                            this->ode.XVars()) +=
        ...
        inline void compute_jacobian_impl(
            ...
            auto Impl = [&](auto& C_XS, auto& C_DXS, auto& C_JDXS, auto& I_XS, auto& I_DXS, auto& I_JDXS,auto & DI_DCS) {
                ...
                for (int i = 0; i < Interiors; i++) {
                    ...
                    DI_DCS(this->ode.TVar(),
                        this->ode.XtUVars()* (Cardinals - 1) + this->ode.TVar()) =
                        Coeffs::InteriorSpacings[i];
                    DI_DCS
                        .template block<DODE::PV, DODE::PV>(
                            this->ode.XtUVars(), Cardinals * this->ode.XtUVars(),
                    ...
                    for (int j = 0; j < Cardinals; j++) {
                        ...
                        DI_DCS.template block<DODE::XV, DODE::XtUV>(
                            0, j * this->ode.XtUVars(), this->ode.XVars(),
                            this->ode.XtUVars()) +=
                        ...
                        DI_DCS.template block<DODE::XV, DODE::PV>(
                            0, Cardinals * this->ode.XtUVars(), this->ode.XVars(),
                            this->ode.PVars()) +=
                            (Coeffs::Cardinal_DXInterp_Weights[i][j] * h) *
                        ...
                        DI_DCS
                            .col(this->ode.XtUVars() * (Cardinals - 1) + this->ode.TVar())
                            .template segment<DODE::XV>(0, this->ode.XVars()) +=
                            Scalar(Coeffs::Cardinal_DXInterp_Weights[i][j]) * C_DXS[j];
                        ...
                        DI_DCS
                            .template block<DODE::UV, DODE::UV>(
                                this->ode.XtVars(),
                                j * this->ode.XtUVars() + this->ode.XtVars(), this->ode.UVars(),
                                this->ode.UVars())
                            .diagonal()
                        ...
                        jx.template block<DODE::XV, DODE::PV>(
                            i * this->ode.XVars(), Cardinals * this->ode.XtUVars(),
                            this->ode.XVars(), this->ode.PVars()) +=
                            (Coeffs::Cardinal_DXDef_Weights[i][j] * h) *
                            C_JDXS[j].template rightCols<DODE::PV>(this->ode.PVars());

                        jx.col(this->ode.TVar())
                            .template segment<DODE::XV>(i * this->ode.XVars(),
                                this->ode.XVars()) -=
                    ...
                    fx.template segment<DODE::XV>(i * this->ode.XVars(), this->ode.XVars()) +=
                        (h * Coeffs::Interior_DXDef_Weights[i] * I_DXS[i]);
                    jx.template middleRows<DODE::XV>(i * this->ode.XVars(),
                        this->ode.XVars()) +=
        ...
        template <class InType, class OutType, class JacType, class AdjGradType,
            ...
            auto Impl = [&](
                ...
                for (int i = 0; i < Interiors; i++) {
                    ...
                    I_AVS[i] = adjvars.template segment<DODE::XV>(i * this->ode.XVars(),
                        this->ode.XVars());
                    ...
                    for (int j = 0; j < Cardinals; j++) {
                        ...
                        C_AVS[j] += I_AGXS[i].template head<DODE::XV>(this->ode.XVars()) *
                            (scale * h * h);
                ...
                for (int j = 0; j < Cardinals; j++) {
                    ...
                    adjhess.template block<DODE::XtUV, DODE::XtUV>(
                        ...
                        this->ode.XtUVars()) +=
                        C_HDXS.template block<DODE::XtUV, DODE::XtUV>(
                            0, 0, this->ode.XtUVars(), this->ode.XtUVars());
                    adjhess.template block<DODE::XtUV, DODE::PV>(
                        j * this->ode.XtUVars(), Cardinals * this->ode.XtUVars(),
                    ...
                    adjhess.template block<DODE::PV, DODE::XtUV>(
                        Cardinals * this->ode.XtUVars(), j * this->ode.XtUVars(),
                        this->ode.PVars(), this->ode.XtUVars()) +=
                        C_HDXS.template block<DODE::PV, DODE::XtUV>(
                            this->ode.XtUVars(), 0, this->ode.PVars(), this->ode.XtUVars());
                    adjhess.template bottomRightCorner<DODE::PV, DODE::PV>(
                        this->ode.PVars(), this->ode.PVars()) +=
                        C_HDXS.template bottomRightCorner<DODE::PV, DODE::PV>(
                            this->ode.PVars(), this->ode.PVars());
                    HTpar.template segment<DODE::XtUV>(j * this->ode.XtUVars(),
                        this->ode.XtUVars()) +=
                        C_AGXS.template segment<DODE::XtUV>(0, this->ode.XtUVars()) *
                        (1.0 / h);
                    ...
                    if constexpr (DODE::PV >= 0) {
                        ...
                        HTpar.tail(this->ode.PVars()) +=
                            C_AGXS.tail(this->ode.PVars()) * (1.0 / h);
                    }
                }

                for (int i = 0; i < Interiors; i++) {
                    if (i > 0)DI_DCS.setZero();
                    DI_DCS(this->ode.TVar(), this->ode.TVar()) =
                        Scalar(1.0 - Coeffs::InteriorSpacings[i]);
                    DI_DCS(this->ode.TVar(),
                        this->ode.XtUVars() * (Cardinals - 1) + this->ode.TVar()) =
                    ...
                    for (int j = 0; j < Cardinals; j++) {
                        ...
                        DI_DCS.template block<DODE::XV, DODE::XtUV>(0, j * this->ode.XtUVars(),
                            this->ode.XVars(),
                            this->ode.XtUVars()) +=
                            (Coeffs::Cardinal_DXInterp_Weights[i][j] * h) *
                            C_JDXS[j].template leftCols<DODE::XtUV>(this->ode.XtUVars());
                    ...
                    HTpar.noalias() +=
                        ((I_AGXS[i].transpose() * Scalar(Coeffs::Interior_DXDef_Weights[i])) *
                            DI_DCS)
                        .transpose();
                }

                adjhess.col(this->ode.TVar()) -= HTpar;
                adjhess.col(this->ode.TVar() + this->ode.XtUVars() * (Cardinals - 1)) +=
                    HTpar;
                adjhess.row(this->ode.TVar()) -= HTpar;
                adjhess.row(this->ode.TVar() + this->ode.XtUVars() * (Cardinals - 1)) +=
                    HTpar;
