...
void ASSET::OptimalControlProblem::check_functions()
{
    ...
    for (auto& f : this->LinkEqualities) CheckFunc(eq, f);
    for (auto& f : this->LinkInequalities) CheckFunc(iq, f);
    for (auto& f : this->LinkObjectives) CheckFunc(obj, f);
...
void ASSET::OptimalControlProblem::transcribe_links() {
  ...
  for (auto& Eq : this->LinkEqualities) {
    auto VC = this->make_link_Vindex_Cindex(
        Eq.LinkFlag, Eq.PhaseRegFlags, Eq.PhasesTolink, Eq.XtUVars, Eq.OPVars,
        Eq.SPVars, Eq.LinkParams, Eq.Func.ORows(), NextEq);
    this->nlp->EqualityConstraints.emplace_back(
        ConstraintFunction(Eq.Func, VC[0], VC[1]));
    this->numEqFuns++;
  }
  for (auto& Iq : this->LinkInequalities) {
    auto VC = this->make_link_Vindex_Cindex(
        Iq.LinkFlag, Iq.PhaseRegFlags, Iq.PhasesTolink, Iq.XtUVars, Iq.OPVars,
        Iq.SPVars, Iq.LinkParams, Iq.Func.ORows(), NextIq);
    this->nlp->InequalityConstraints.emplace_back(
        ConstraintFunction(Iq.Func, VC[0], VC[1]));
    this->numIqFuns++;
  }
  for (auto& Ob : this->LinkObjectives) {
    int dummy = 0;
    auto VC = this->make_link_Vindex_Cindex(
        Ob.LinkFlag, Ob.PhaseRegFlags, Ob.PhasesTolink, Ob.XtUVars, Ob.OPVars,
        Ob.SPVars, Ob.LinkParams, Ob.Func.ORows(), dummy);
...
void ASSET::OptimalControlProblem::transcribe(bool showstats, bool showfuns) {
  ...
  if (showstats) this->print_stats(showfuns);
  this->nlp->make_NLP(this->numProbVars, this->numProbEqCons,
                      this->numProbIqCons);
...
ASSET::OptimalControlProblem::make_link_Vindex_Cindex(
...
void ASSET::OptimalControlProblem::Build(py::module& m) {
  using namespace doc;
  auto obj = py::class_<OptimalControlProblem,std::shared_ptr<OptimalControlProblem>,OptimizationProblemBase>(m, "OptimalControlProblem");
  obj.def(py::init<>());

  //////////////////
  obj.def("addLinkEqualCon",
          py::overload_cast<LinkConstraint>(
  ...
  obj.def("addLinkEqualCon",
          py::overload_cast<VectorFunctionalX, RegVec, std::vector<VectorXi>,
  ...
  obj.def("addLinkEqualCon",
      py::overload_cast<VectorFunctionalX, RegVec, std::vector<std::vector<std::shared_ptr<ODEPhaseBase>>>,
      std::vector<VectorXi>, std::vector<VectorXi>>(
          &OptimalControlProblem::addLinkEqualCon),
      OptimalControlProblem_addLinkEqualCon2);
  ...
  obj.def("addDirectLinkEqualCon",
      py::overload_cast<VectorFunctionalX, PhasePtr, PhaseRegionFlags, VectorXi, PhasePtr, PhaseRegionFlags, VectorXi>
      (&OptimalControlProblem::addDirectLinkEqualCon),
      OptimalControlProblem_addDirectLinkEqualCon);
  ...
  obj.def("addLinkInequalCon",
      py::overload_cast<VectorFunctionalX, RegVec, std::vector<std::vector<PhasePtr>>,
      std::vector<VectorXi>, std::vector<VectorXi>>(
          &OptimalControlProblem::addLinkInequalCon));
  ...
  obj.def("addLinkObjective",
      py::overload_cast<LinkObjective>(
  ...
  obj.def("addLinkObjective",
      py::overload_cast<ScalarFunctionalX, RegVec, std::vector<VectorXi>,
      std::vector<VectorXi>, std::vector<VectorXi>>(
          &OptimalControlProblem::addLinkObjective),
      OptimalControlProblem_addLinkObjective);
  ...
  obj.def("addLinkObjective",
      py::overload_cast<ScalarFunctionalX, RegVec, std::vector<std::vector<PhasePtr>>,
      std::vector<VectorXi>, std::vector<VectorXi>>(
          &OptimalControlProblem::addLinkObjective),
      OptimalControlProblem_addLinkObjective);
  ...
  obj.def("addLinkParamEqualCon",
          py::overload_cast<VectorFunctionalX, VectorXi>(
              &OptimalControlProblem::addLinkParamEqualCon),
  ...
  obj.def("addPhase", py::overload_cast<PhasePtr>
  ...
  obj.def("removePhase", &OptimalControlProblem::removePhase,
          OptimalControlProblem_removePhase);
  obj.def("Phase", &OptimalControlProblem::Phase, OptimalControlProblem_Phase);
  obj.def("setLinkParams", &OptimalControlProblem::setLinkParams,
  ...
  obj.def("addLinkEqualCon",
      py::overload_cast<VectorFunctionalX,
      int , std::string , Eigen::VectorXi , Eigen::VectorXi , Eigen::VectorXi ,
      int , std::string, Eigen::VectorXi, Eigen::VectorXi, Eigen::VectorXi,
      VectorXi>(&OptimalControlProblem::addLinkEqualCon));

  obj.def("addLinkEqualCon",
      py::overload_cast<VectorFunctionalX,
      PhasePtr, std::string, Eigen::VectorXi, Eigen::VectorXi, Eigen::VectorXi,
      PhasePtr, std::string, Eigen::VectorXi, Eigen::VectorXi, Eigen::VectorXi,
      VectorXi>(&OptimalControlProblem::addLinkEqualCon));
  ...
  obj.def("addLinkEqualCon",
      py::overload_cast<VectorFunctionalX,
      int, std::string, Eigen::VectorXi,
      int, std::string, Eigen::VectorXi
      >(&OptimalControlProblem::addLinkEqualCon));

  obj.def("addLinkEqualCon",
      py::overload_cast<VectorFunctionalX,
      PhasePtr, std::string, Eigen::VectorXi,
      PhasePtr, std::string, Eigen::VectorXi
      >(&OptimalControlProblem::addLinkEqualCon));
