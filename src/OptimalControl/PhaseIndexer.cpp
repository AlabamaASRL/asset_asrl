...
int ASSET::PhaseIndexer::addEquality(ConstraintInterface eqfun,
  ...
  auto vincin = this->make_Vindex_Cindex(sreg, rxtuv, rodepv, rstatpv,
                                         eqfun.ORows(), this->nextPhaseEqCon);
  this->nlp->EqualityConstraints.emplace_back(
      ConstraintFunction(eqfun, vincin[0], vincin[1]));
...
void ASSET::PhaseIndexer::addPartitionedEquality(
    const std::vector<ConstraintInterface>& eqfuns, PhaseRegionFlags sreg,
    const Eigen::VectorXi& rxtuv, const Eigen::VectorXi& rodepv,
    const Eigen::VectorXi& rstatpv, const std::vector<int>& Tmodes) {
  int index = this->nlp->EqualityConstraints.size();
  int temp = this->nextPhaseEqCon;
  auto vincin = this->make_Vindex_Cindex(
      sreg, rxtuv, rodepv, rstatpv, eqfuns[0].ORows(), this->nextPhaseEqCon);
...
int ASSET::PhaseIndexer::addAccumulation(
    ConstraintInterface eqfun, PhaseRegionFlags sreg,
    const Eigen::VectorXi& rxtuv, const Eigen::VectorXi& rodepv,
...

{
  ...
  auto vincinAcc =
      this->make_Vindex_Cindex(PhaseRegionFlags::Params, empty, empty, accpv,
                               accfun.ORows(), this->nextPhaseEqCon);
  this->nlp->EqualityConstraints.emplace_back(
      ConstraintFunction(accfun, vincinAcc[0], vincinAcc[1]));
  this->nlp->EqualityConstraints.back().ThreadMode = Tmode;
...
int ASSET::PhaseIndexer::addInequality(ConstraintInterface iqfun,
  ...
  auto vincin = this->make_Vindex_Cindex(sreg, rxtuv, rodepv, rstatpv,
                                         iqfun.ORows(), this->nextPhaseIqCon);
...
void ASSET::PhaseIndexer::addPartitionedInequality(
  ...
  int temp = this->nextPhaseIqCon;
  auto vincin = this->make_Vindex_Cindex(
      sreg, rxtuv, rodepv, rstatpv, iqfuns[0].ORows(), this->nextPhaseIqCon);
...

{
  ...
  auto vincin =
      this->make_Vindex_Cindex(sreg, rxtuv, rodepv, rstatpv, objfun.ORows());
...
std::array<Eigen::MatrixXi, 2> ASSET::PhaseIndexer::make_Vindex_Cindex(PhaseRegionFlags sreg, const VectorXi& rxtuv, const VectorXi& rodepv, const VectorXi& rstatpv, int orows, int& NextCLoc) const
{
    ...
    int blockdeflen =
        DefectCardinalStates * this->XtVars() + this->UVars() + this->PVars();

    int xpblen = DefectCardinalStates * xsize +
        (DefectCardinalStates - 1) * xsize + opsize + spsize;
    ...
    for (int i = 0; i < rxtuv.size(); i++) {
        if (rxtuv[i] < this->XtVars()) IsOnlyControl = false;
    }

    auto VinLoopImpl = [&](int& loc, int col, const VectorXi& Locs,
        const VectorXi& Vars) {
    ...
    case PhaseRegionFlags::DefectPath: {
        ...
        for (int i = 0; i < this->numDefects; i++) {
            ...
            for (int j = 0; j < this->DefectCardinalStates; j++) {
                DefectPathLoopImpl(loc, i, i * (this->DefectCardinalStates - 1) + j,
                    i);
            }
            VinLoopImpl(loc, i, this->ODEParamLocs, rodepv);
            VinLoopImpl(loc, i, this->StaticParamLocs, rstatpv);
        ...
        for (int i = 0; i < this->numDefects; i++) {
            ...
            for (int j = 0; j < this->DefectCardinalStates; j++) {
                for (int k = 0; k < this->XtVars(); k++) {
                    Vindex(loc, i) = this->getXTUVarLoc(
                        k, i * (this->DefectCardinalStates - 1) + j, i);
                    loc++;
                }
            }
            for (int k = this->XtVars(); k < this->XtUVars(); k++) {
                Vindex(loc, i) =
                    this->getXTUVarLoc(k, i * (this->DefectCardinalStates - 1), i);
                loc++;
            }
            VinLoopImpl(loc, i, this->ODEParamLocs, rodepv);
            // VinLoopImpl(loc, i, this->StaticParamLocs, rstatpv);
...
Eigen::VectorXd ASSET::PhaseIndexer::makeSolverInput(const std::vector<Eigen::VectorXd>& ActiveTraj, const Eigen::VectorXd& ActiveStaticParams) const
{
    Eigen::VectorXd Vars(this->numPhaseVars);

    if (this->BlockedControls) {
        for (int i = 0; i < this->numStates; i++) {
            Vars.segment(i * this->XtVars(), this->XtVars()) =
                ActiveTraj[i].head(this->XtVars());
        }
        for (int i = 0; i < this->numDefects; i++) {
            Vars.segment(this->numStates * this->XtVars() + i * this->UVars(),
                this->UVars()) =
                ActiveTraj[i * (this->DefectCardinalStates - 1)].segment(
                    this->XtVars(), this->UVars());
        }

        if (this->PVars() > 0) {
            Vars.segment(
                this->numStates * this->XtVars() + this->numDefects * this->UVars(),
        ...
        if (this->PVars() > 0) {
            Vars.segment(this->numStates * this->XtUVars(), this->PVars()) =
                ActiveTraj[0].tail(this->PVars());
        }
        if (this->StatPVars() > 0) {
            Vars.segment(this->numStates * this->XtUVars() + this->PVars(),
                this->StatPVars()) = ActiveStaticParams;
        }
    }
...
void ASSET::PhaseIndexer::collectSolverOutput(const Eigen::VectorXd& Vars, std::vector<Eigen::VectorXd>& ActiveTraj, Eigen::VectorXd& ActiveStaticParams) const
{
    ...
    if (this->BlockedControls) {
        ...
        for (int i = 0; i < this->numStates; i++) {
            ...
            ActiveTraj[i].segment(this->XtVars(), this->UVars()) = Vars.segment(
                this->numStates * this->XtVars() + unum * this->UVars(),
                this->UVars());
...
std::vector<Eigen::VectorXd> ASSET::PhaseIndexer::getFuncEqMultipliers(int Gindex, const Eigen::VectorXd& EMultphase) const
{
    Eigen::MatrixXi CDX =
        this->nlp->EqualityConstraints[Gindex].index_data.Cindex;
...
std::vector<Eigen::VectorXd> ASSET::PhaseIndexer::getFuncIqMultipliers(int Gindex, const Eigen::VectorXd& IMultphase) const
{
    Eigen::MatrixXi CDX =
        this->nlp->InequalityConstraints[Gindex].index_data.Cindex;
    std::vector<Eigen::VectorXd> mults;
    for (int i = 0; i < CDX.cols(); i++) {
        Eigen::VectorXd lmi(CDX.rows());
        for (int j = 0; j < CDX.rows(); j++) {
...
void ASSET::PhaseIndexer::Test()
{
    ...
    std::cout << " Front" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::Front, xv, pv, sv, 1)[0]
        << endl
        << endl;

    std::cout << " Back" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::Back, xv, pv, sv, 1)[0]
        << endl
        << endl;

    std::cout << " FrontandBack" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::FrontandBack, xv, pv,
        sv, 1)[0]
        << endl
        << endl;

    std::cout << " BackandFront" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::BackandFront, xv, pv,
        sv, 1)[0]
        << endl
        << endl;

    std::cout << " Path" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::Path, xv, pv, sv, 1)[0]
        << endl
        << endl;

    std::cout << " InnerPath" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::InnerPath, xv, pv, sv,
        1)[0]
        << endl
        << endl;

    std::cout << " NodalPath" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::NodalPath, xv, pv, sv,
        1)[0]
        << endl
        << endl;

    std::cout << " PairwisePath" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::PairWisePath, xv, pv,
        sv, 1)[0]
        << endl
        << endl;

    std::cout << " FrontNodalBackPath" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::FrontNodalBackPath, xv,
        pv, sv, 1)[0]
        << endl
        << endl;

    std::cout << " DefectPath" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::DefectPath, xv, pv, sv,
        1)[0]
        << endl
        << endl;

    std::cout << " DefectPairWisePath" << endl;
    std::cout << s.make_Vindex_Cindex(PhaseRegionFlags::DefectPairWisePath, xv,
        pv, sv, 1)[0]
        << endl
        << endl;
}
