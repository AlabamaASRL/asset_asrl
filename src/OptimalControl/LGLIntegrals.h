...
struct LGLReducedInteg_Impl {
  ...
  struct Weight : StaticScaleBase<Weight<Elem>> {
    static constexpr double value =
        LGLCoeffs<CS>::Reduced_Integral_Weights[Elem];
  };
  ...
  static auto Definition(const Integrand& integ, int xv, int pv) {
    ...
    auto xtf = Args.template segment<XTV, SZ_PROD<CS - 1, XTV>::value>(
        (CS - 1) * xtv, xtv);
    ...
    auto integral = constexpr_forwarding_loop(
        ...
        [&](auto i, auto tup) {
          auto xti = Args.template segment<XTV, SZ_PROD<i.value, XTV>::value>(
              i.value * xtv, xtv);
          auto xi = [&]() {
            if constexpr (PV == 0)
