    ...
    for (int i = 0; i < this->NumBlocks; i++) {
        ...
        if (i > 0 && i < (this->NumBlocks - 1)) {
            ...
            err = ((yvecs[i] - yvecs[i - 1]) / (hs[i] + hs[i - 1])
                + (yvecs[i] - yvecs[i + 1]) / (hs[i] + hs[i + 1])).lpNorm<Eigen::Infinity>();
        }
        else if (i == 0) {
...
void ASSET::LGLInterpTable::DeboorMeshError(Eigen::VectorXd& tsnd, Eigen::MatrixXd& mesh_errors, Eigen::MatrixXd& mesh_dist) const
{
