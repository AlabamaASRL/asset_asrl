...
std::vector<VectorType> FDiffData(const std::vector<VectorType>& BL, int axis,
                                  bool inctime) {
  ...
  for (int i = 0; i < BL.size(); i++) {
    ...
    if (i < 2) {
      BLDot[i].head(axis) =
          ((1.0 / 3.0) * BL[i + 3].head(axis) - 1.5 * BL[i + 2].head(axis) +
           3.0 * BL[i + 1].head(axis) - (11.0 / 6) * BL[i].head(axis)) *
          (1.0 / h);
    } else if (i > (BL.size() - 3)) {
      BLDot[i].head(axis) = (BL[i].head(6) - BL[i - 1].head(axis)) * (1.0 / h);
    } else {
      BLDot[i].head(axis) = ((-1.0 / 12) * BL[i + 2].head(axis) +
                             (2.0 / 3.0) * BL[i + 1].head(axis) +
                             (-2.0 / 3.0) * BL[i - 1].head(axis) +
                             (1.0 / 12.0) * BL[i - 2].head(axis)) *
                            (1.0 / h);
    }
  }
  return BLDot;
...
struct FDDerivArbitrary {
 public:
  ...
  inline void derivAt(const int i, DerivType& dout,const int Order,const int Accuracy) const {
    ...
    const int centStenSize =
        ((ord / 2) == ((ord + 1) / 2)) ? (ord - 1 + acc) : (ord + acc);
    const int fbStenSize = acc + ord;

    if (length < fbStenSize) {
      std::cout << "ERROR: Requested accuracy too high for given data"
                << std::endl;
    }
