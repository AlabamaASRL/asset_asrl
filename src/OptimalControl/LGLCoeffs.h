...
struct LGLCoeffs<2> {  /// Cubic Hermite-LGL3
  ...
  static constexpr STDarray<double, 2> Cardinal_Integral_Weights = {1.0 / 3.0,
                                                                    1.0 / 3.0};
  ...
  static constexpr STDarray<STDarray<double, 4>, 2> Cardinal_XPower_Weights = {
      STDarray<double, 4>{2.0, -3.0, 0.0, 1.0},
      STDarray<double, 4>{-2.0, 3.0, 0.0, 0.0}};

  static constexpr STDarray<STDarray<double, 4>, 2> Cardinal_DXPower_Weights = {
      STDarray<double, 4>{1.0, -2.0, 1.0, 0.0},
      STDarray<double, 4>{1.0, -1.0, 0.0, 0.0}};
...
struct LGLCoeffs<3> {  /// Cubic Hermite-LGL3
  ...
  static constexpr STDarray<double, 2> InteriorSpacings = {0.172673164646011,
                                                           0.827326835353989};
  ...
  static constexpr STDarray<STDarray<double, 3>, 2> Cardinal_UPoly_Weights = {
      STDarray<double, 3>{0.541612549639704, 0.571428571428571,
                          -0.113041121068274},
      STDarray<double, 3>{-0.113041121068274, 0.571428571428571,
                          0.541612549639704},
  };
  ...
  /// Weights for Integrals
  static constexpr STDarray<double, 3> Cardinal_Integral_Weights = {
      0.1, 0.7111111111111111111111, 0.1};
  static constexpr STDarray<double, 2> Interior_Integral_Weights = {
      0.544444444444444444444, 0.544444444444444444444};

  static constexpr STDarray<double, 3> Reduced_Integral_Weights = {
  ...
  static constexpr STDarray<STDarray<double, 3>, 3>
      Cardinal_UPolyPower_Weights = {
...
struct LGLCoeffs<4> {
  ...
  static constexpr STDarray<STDarray<double, 4>, 3> Cardinal_UPoly_Weights = {
      STDarray<double, 4>{0.550643660407289, 0.551767574740443,
                          -0.153490305524281, 0.0510790703765507},
      STDarray<double, 4>{-0.140877081724073, 0.640877081724073,
                          0.640877081724073, -0.140877081724073},
  ...
  static constexpr STDarray<STDarray<double, 8>, 4> Cardinal_XPower_Weights = {
      STDarray<double, 8>{322.113192893432, -1262.16647180554,
                          +1953.18722397597, -1497.44073672143,
                          +575.419724269949, -92.112932612382, 0.0, +1.0},
      STDarray<double, 8>{-64.79204848488, +361.542466375583, -764.578109564441,
                          +777.477964267544, -383.431222919698,
                          73.7809503258911, 0.0, 0.0},
      STDarray<double, 8>{64.7920484849059, -92.0018730186832,
                          -44.0436705064007, +110.002715108135,
                          -43.8271690468644, +5.07794897890751, 0.0, 0.0},
      STDarray<double, 8>{-322.11319289346, +992.625878448649,
                          -1144.56544390514, +609.960057345752,
                          -148.161332303388, +13.2540333075835, 0.0, 0.0}};

  static constexpr STDarray<STDarray<double, 8>, 4> Cardinal_DXPower_Weights = {
      STDarray<double, 8>{26.2862997682608, -105.145199073049,
                          +167.971831917166, -135.907298995812,
                          +58.0483996910198, -12.254033307585, +1.0, 0.0},
      STDarray<double, 8>{119.581459799146, -446.567920871157,
                          +645.538793826344, -446.829224641783,
                          +145.40645229346, -17.1295604060043, 0.0, 0.0},
      STDarray<double, 8>{119.581459799146, -390.502297722867,
                          +477.341924381445, -267.69702439259,
                          +67.4701675365851, -6.19422960171859, 0.0, 0.0},
      STDarray<double, 8>{26.2862997682629, -78.8588993047889,
                          +89.1129326123729, -46.7943663834307,
                          +11.2540333075837, -0.999999999999869, 0.0, 0.0}};
  static constexpr STDarray<double, 4> Reduced_Integral_Weights = {
      -5.12701665379258 / 4.0 + 10.2540333075852 / 3.0 -
          6.12701665379258 / 2.0 + 1.0,
  ...
  static constexpr STDarray<STDarray<double, 4>, 4>
      Cardinal_UPolyPower_Weights = {
          STDarray<double, 4>{-5.12701665379258, +10.2540333075852,
                              -6.12701665379258, +1.0},
          STDarray<double, 4>{10.9353308042859, -18.9665045333251,
                              +8.03117372903925, 0.0},
          STDarray<double, 4>{-10.9353308042859, +13.8394878795326,
                              -2.90415707524666, 0.0},
  ...
  static constexpr STDarray<STDarray<double, 4>, 2> UZeroSpline_Weights = {
      STDarray<double, 4>{-6.12701665379258, 8.03117372903925,
                          -2.90415707524666, 1.0},
      STDarray<double, 4>{10.2540333075852 * 2.0, -18.9665045333251 * 2.0,
                          +13.8394878795326 * 2.0, -5.12701665379258 * 2.0}};

  static constexpr STDarray<STDarray<double, 4>, 2> UOneSpline_Weights = {
      STDarray<double, 4>{
          -5.12701665379258 * 3.0 + 10.2540333075852 * 2.0 - 6.12701665379258,
