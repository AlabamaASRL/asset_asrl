...
struct FDCoeffs<2, 6, FDCoeffType::Forwards, 0> {
  ...
  static constexpr arr<N> weights = {469 / 90.0,   -223 / 10.0, 879 / 20.0,
                                     -949 / 18.0,  41,          -201 / 10.0,
                                     1019 / 180.0, -7 / 10.0};
};
...
struct FDCoeffs<4, 4, FDCoeffType::Forwards, 0> {
  static constexpr int N = 8;
  static constexpr arr<N> weights = {28 / 3.0,    -111 / 2.0, 142,
                                     -1219 / 6.0, 176,        -185 / 2.0,
                                     82 / 3.0,    -7 / 2.0};
};
...
struct FDCoeffs<2, 6, FDCoeffType::Forwards, 1> {
  static constexpr int N = 8;
  static constexpr arr<N> weights = {7 / 10.0, -7 / 18.0,  -27 / 10.0,
                                     19 / 4.0, -67 / 18.0, 9 / 5.0,
                                     -1 / 2.0, 11 / 180.0};
};
...
struct FDCoeffs<3, 6, FDCoeffType::Forwards, 1> {
  static constexpr int N = 9;
  static constexpr arr<N> weights = {-469 / 240.0, 303 / 40.0, -731 / 60.0,
                                     269 / 24.0,   -57 / 8.0,  407 / 120.0,
                                     -67 / 60.0,   9 / 40.0,   -1 / 48.0};
};
...
struct FDCoeffs<4, 4, FDCoeffType::Forwards, 1> {
  static constexpr int N = 8;
  static constexpr arr<N> weights = {7 / 2.0,   -56 / 3.0, 85 / 2.0, -54,
                                     251 / 6.0, -20,       11 / 2.0, -2 / 3.0};
};
...
struct FDCoeffs<2, 6, FDCoeffType::Forwards, 2> {
  static constexpr int N = 8;
  static constexpr arr<N> weights = {-11 / 180.0, 107 / 90.0, -21 / 10.0,
                                     13 / 18.0,   17 / 36.0,  -3 / 10.0,
                                     4 / 45.0,    -1 / 90.0};
};
...
struct FDCoeffs<3, 6, FDCoeffType::Forwards, 2> {
  static constexpr int N = 9;
  static constexpr arr<N> weights = {-1 / 48.0,   -53 / 30.0, 273 / 40.0,
                                     -313 / 30.0, 103 / 12.0, -9 / 2.0,
                                     197 / 120.0, -11 / 30.0, 3 / 80.0};
};
...
struct FDCoeffs<4, 4, FDCoeffType::Forwards, 2> {
  static constexpr int N = 8;
  static constexpr arr<N> weights = {2 / 3.0,   -11 / 6.0, 0,        31 / 6.0,
                                     -22 / 3.0, 9 / 2.0,   -4 / 3.0, 1 / 6.0};
};
...
struct FDCoeffs<1, 8, FDCoeffType::Central, 0> {
  static constexpr int N = 9;
  static constexpr arr<N> weights = {1 / 280.0, -4 / 105.0, 1 / 5.0,
                                     -4 / 5.0,  0,          4 / 5.0,
                                     -1 / 5.0,  4 / 105.0,  -1 / 280.0};
};
...
struct FDCoeffs<2, 8, FDCoeffType::Central, 0> {
  static constexpr int N = 9;
  static constexpr arr<N> weights = {-1 / 560.0, 8 / 315.0,   -1 / 5.0,
                                     8 / 5.0,    -205 / 72.0, 8 / 5.0,
...
struct FDCoeffs<3, 6, FDCoeffType::Central, 0> {
  static constexpr int N = 9;
  static constexpr arr<N> weights = {-7 / 240.0,  3 / 10.0,  -169 / 120.0,
                                     61 / 30.0,   0,         -61 / 30.0,
                                     169 / 120.0, -3 / 10.0, 7 / 240.0};
};
...
struct FDCoeffs<4, 6, FDCoeffType::Central, 0> {
  static constexpr int N = 9;
  static constexpr arr<N> weights = {7 / 240.0,   -2 / 5.0, 169 / 60.0,
                                     -122 / 15.0, 91 / 8.0, -122 / 15.0,
                                     169 / 60.0,  -2 / 5.0, 7 / 240.0};
};
