...
def AnimSlew(traj,sp=6,Qsig=[],fname = 'Video',Anim=True,Elev=30,Azim =0,Ivec=[1,1,1], scaleH=True,time=10,save=True):
    BT =[]
    Ivec = np.array(Ivec)
    TT = np.array(traj).T
    xbasis = []
    ...
    def init():
        ...
        return WX,WY,WZ,TX,TY,TZ,XT,YT,ZT,XTT,YTT,ZTT
    def animate(i):
    ...
    fig.tight_layout()
    if(Anim==True):
        anim = animation.FuncAnimation(fig, animate, init_func=init,frames=len(traj), interval=time*1000/len(traj), blit=True)
    #anim.save('basic_animation.html', fps=30, extra_args=['-vcodec', 'libx264'])
    
    if(save):
        FFwriter = animation.FFMpegWriter( fps=30)
        anim.save('man.mp4', writer = FFwriter,dpi=300)
...
def PlotSlew2(traj,sp=6,Qsig=[],fname = 'Video',Anim=True,Elev=30,Azim =0,Ivec=[1,1,1], scaleH=True):
    BT =[]
    Ivec = np.array(Ivec)
    TT = np.array(traj).T
    xbasis = []
    ...
    TX, =axs[0].plot(TT[Axis],BT[0],label=r'$q_1$', color="r")
    TY, =axs[0].plot(TT[Axis],BT[1],label=r'$q_2$', color="g")
    TZ, =axs[0].plot(TT[Axis],BT[2],label=r'$q_3$', color="b")
    TW, =axs[0].plot(TT[Axis],BT[3],label=r'$q_4$', color="k")
    
    axs[0].grid(True)
    axs[0].set_ylabel(r'$\bf{q}$')
...
def CompSlew(trajs,sp=6,Qsig=[],fname = 'Video',Anim=True,Elev=30,Azim =0,Ivec=[1,1,1],nvec=[1,1,1], scaleH=True):
    ...
    legend1 = plt.legend([lines[i] for i in [0,3,6]], ["OPT", "MAM", "EAM"], loc=1)
