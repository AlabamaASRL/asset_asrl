...
class ShuttleReentry(oc.ODEBase):
    def __init__(self,table,npoints=1000):
        
        Xvars = 5
        Uvars = 2
        
        
        ## Sample an exponential model
        hs = np.linspace(-2000/Lstar,400000/Lstar,npoints)
        
        hs = list(hs)
        rohs = np.array([rho0*np.exp(-h/h_ref) for h in hs])
        rtab = vf.InterpTable1D(hs,rohs)
        ...
        # Replace analytic function with table
        if(table):    
            rho = rtab(h)
...
def Kernel(tmode,table,npoints):
    ...
    ode = ShuttleReentry(table,npoints)
    
    
    phase = ode.phase(tmode,TrajIG,16)
    ...
    phase.addLUVarBound("Path",0,0,ht0*1.5,1.0)
    ...
    phase.setMeshTol(1.0e-10)
    phase.optimizer.EContol=1.0e-10 #Set EContol at least as tight as MeshTol
    ...
    #Plot(phase.returnTraj(),phase.returnTraj())
    return phase.getMeshIters()[-1]
...
if __name__ == "__main__":
    ...
    npoints = [200,500,1000,2000,16000]
    tmode = "LGL3"  # look at 3 5 and 7
    for i,npoint in enumerate(npoints):
        # Use a curve fit of exponential model with increasing # of sample points points
        miter = Kernel(tmode,True,npoint)
        hs = np.array(miter.times[1:]-miter.times[0:-1])
        scale =  sum( np.array(miter.distribution[0:-1])*hs)
        print(scale)
        ## This plots the error in the last iterate on nondim time interval
        plt.plot(miter.times,miter.distribution/scale,label=f"Num Points = {npoint}")
    
    
    
    # Use the exact exponential model
    miter = Kernel(tmode,False,50)
    hs = np.array(miter.times[1:]-miter.times[0:-1])
    scale =  sum( np.array(miter.distribution[0:-1])*hs)
    print(scale)
    plt.plot(miter.times,miter.distribution/scale,label=f"Exact Model",color='k',linestyle = 'dotted')
    #plt.yscale("log")
    plt.xlabel("t (0-1)")
