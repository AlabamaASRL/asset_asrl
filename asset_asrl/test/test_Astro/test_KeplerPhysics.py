...
def specific_angular_momentum(state):
    ...
    body. Consequently, the specific angular momentum vector is conserved
...
class KeplerPropagatorPhysicsTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        The gravitational parameter is set to mu = 1 to create a
        nondimensional two-body problem.
        ...
        the required circular velocity is also
        ...
        states for several of the regression tests.
        """
    ...
    def propagate(self, state, duration):
        ...
        This helper constructs that seven-element input vector, evaluates
        the underlying ASSET VectorFunction, and converts the result back to
        a NumPy array.

        A positive duration propagates the state forward in time, while
        a negative duration propagates it backward in time.
        """
    ...
    def test_circular_orbit_advances_by_a_quarter_period(self):
        ...
        For mu = 1 and a unit-radius circular orbit, the initial state
        ...
        This test verifies that the ASSET Kepler propagator reproduces that
    ...
    def test_two_body_energy_and_angular_momentum_are_conserved(self):
        ...
        In an ideal two-body gravitational problem, the gravitational force
        is conservative and central. Consequently, both specific mechanical
        energy and specific angular momentum are constants of the motion.
        ...
        self.assertAlmostEqual(specific_energy(final_state, self.mu), specific_energy(initial_state, self.mu),)
        np.testing.assert_allclose(specific_angular_momentum(final_state), specific_angular_momentum(initial_state))
    ...
    def test_forward_then_backward_propagation_recovers_initial_state(self):
        ...
        Because the ASSET Kepler propagator uses an analytical two-body
        solution rather than a conventional numerical time-stepping scheme,
        the recovered state should agree with the original state to very high
        precision.
        """
