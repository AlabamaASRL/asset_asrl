...
if __name__ == "__main__":
    ...
    ocp.addForwardLinkEqualCon(0, 1, [0, 1, 2, 6])

    def MatchDV(X=Args(9)):
        V1 = X.head3()
        V2 = X.segment3(3)
        DV = X.tail3()
    ...
    # ocp.addLinkObjective(VinfObj(),oc.LinkFlags.BackToFront,[[0,1],[1,2],[2,3]],[3,4,5])

    ocp.addLinkEqualCon(VinfMatchCon(MarsTab),
                        oc.LinkFlags.BackToFront, [[0, 1]], [3, 4, 5, 6])

    ocp.addLinkInequalCon(FlybyAngleBound(
        MarsTab, mu_Val_Mars, r_Mars_Psyche), oc.LinkFlags.BackToFront, [[0, 1]], [3, 4, 5, 6])
