...
import asset.VectorFunctions as vf
...
def OctSubPLot(fig1, ax1, points, zs, Ivec, EqualColor=True, cmap='PuOr', Legend=False, badpoints=[]):
    ...
    tpc = ax1.tripcolor(triang, zs, shading='gouraud',
                        cmap=cmap, vmin=vmin, vmax=vmax, alpha=.9)
    #tpc.set_clim(min(zs), max(zs))

    cbar = fig1.colorbar(tpc, ax=ax1, pad=0.007,
                         boundaries=np.linspace(min(zs), max(zs), 60))
    ...
    '''
    ticks = list(np.linspace(min(zs)*.99, max(zs)*1.2, 6)
                 )  # +list([ max(zs)*.995])
    cbar.set_ticks(ticks)
    cbar.ax.set_yticklabels(['{:.1f}'.format(x) + "%" for x in ticks])

    # cbar.ax.locator_params(nbins=3)
    cbar.set_label(r'$\Delta$', rotation=0, fontsize=12)

    zz = np.linspace(0, 1, len(zs))
    # ax1.scatter(pp[1],pp[2],c=zz,cmap='viridis',zorder=10,s=.5)

    ax1.scatter(xyz[0][1], xyz[0][2], color='red',
                label=r'$\hat{X}(0)$', zorder=10, edgecolor='k')
    ax1.scatter(xyz[1][1], xyz[1][2], color='green',
                label=r'$\hat{Y}(0)$', zorder=10, edgecolor='k')
    ax1.scatter(xyz[2][1], xyz[2][2], color='blue',
                label=r'$\hat{Z}(0)$', zorder=10, edgecolor='k')
    ...
    ax1.scatter(sp[1], sp[2], color='k', marker='.',
                s=.6, label=r'$\hat{n}_i$')
    if(Legend == True):
        ax1.legend()
    ...
    ax1.plot(XLats[1], XLats[2], color='k',
             linestyle='--', alpha=.7, linewidth=0.6)
    ax1.plot(YLats[1], YLats[2], color='k',
             linestyle='--', alpha=.7, linewidth=0.6)
    ax1.plot(ZLons[1], ZLons[2], color='k',
             linestyle='--', alpha=.7, linewidth=0.6)
...
def EigAxisPhase(Ivec, Tmax, nvec, theta, Nsegs):
    ...
    phase.JetJobMode = solvs.JetJobModes.Optimize

    return phase


def FullAxisPhase(Ivec, Tmax, nvec, theta, EigAxPhase, Nsegs):
...
def CalcManeuvers(Ivec, thetadeg, Tmax=1, n=1000, nsegs=200, ccount=8):
    nvecs = octant_points(n)
    nvecs.append(normalize([1, 1, 1]))
    theta = np.deg2rad(thetadeg)

    EAArgs = [(Ivec, Tmax, np.array(nvec), theta, nsegs) for nvec in nvecs]

    #tmp =[EigAxisPhase(*x) for x in EAArgs]
    
    #EigAxisRes = solvs.Jet.map(tmp, ccount)
    
    EigAxisRes = solvs.Jet.map(EigAxisPhase, EAArgs, ccount)

    FAArgs = [(Ivec, Tmax, nvec, theta, EigAx, nsegs)
              for nvec, EigAx in zip(nvecs, EigAxisRes)]
    
    #tmp =[FullAxisPhase(*x) for x in FAArgs]
    #FullAxisRes = solvs.Jet.map(tmp, ccount)
    
    FullAxisRes = solvs.Jet.map(FullAxisPhase, FAArgs, ccount)
    ...
    for F in FullAxisRes:
        Inum = F.optimizer.LastIterNum
        tts.append(F.optimizer.LastTotalTime)
        qps.append(F.optimizer.LastKKTTime/tts[-1])
        us.append(F.optimizer.LastFuncTime/tts[-1])
        pts.append(F.optimizer.LastPreTime/tts[-1])
        mts.append(F.optimizer.LastMiscTime/tts[-1])
        
        #print(qps[-1] + us[-1] + pts[-1] + mts[-1])
    
    for i in range(2,len(qps)-2):
        qps[i] = sum(qps[i-2:i+3])/5
        us[i] = sum(us[i-2:i+3])/5
        pts[i] = sum(pts[i-2:i+3])/5
        
    
    #plt.plot(range(0,len(qps)),tts)
    plt.plot(range(0,len(qps)),qps)
    plt.plot(range(0,len(qps)),us)
    plt.plot(range(0,len(qps)),pts)
    plt.plot(range(0,len(qps)),mts)
    ...
    for i in range(0, len(FullAxisRes)):

        TrajEig = EigAxisRes[i].returnTraj()
        tfeig = TrajEig[-1][2]

        TrajFull = FullAxisRes[i].returnTraj()
        tffull = TrajFull[-1][7]
    ...
    FullAxisRes[-1].solve()
    EigAxisRes[-1].solve()
    TrajFull = FullAxisRes[-1].returnTraj()[1:-1]
    ...
    OctSubPLot(fig, ax1, nvecs, zs, Ivec, cmap='plasma',
               EqualColor=False, Legend=True)
...
if __name__ == "__main__":
    ...
    CalcManeuvers(Ivec, 120.0, n=3000, nsegs=250, ccount=3)
