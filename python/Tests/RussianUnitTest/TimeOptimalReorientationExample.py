...
def CalcManeuver(Ivec,nvec,thetadeg,Tmax=1):
    ...
    OPT = FullAxis(Ivec,Tmax,nvec,theta,EAM,384)
    
    AnimSlew(EAM[1:-1],Anim=True,Ivec=Ivec)
    AnimSlew(OPT[1:-1],Anim=True,Ivec=Ivec)
    
if __name__ == "__main__":
    
    Ivec = np.array([1,2.0,2.6])    ## Inertia of a 6U cubesat
        
    CalcManeuver(Ivec,[1,1,1],170.0)
    CalcManeuver(Ivec,[1,2,2],150.0)
    CalcManeuver(Ivec,[1,.1,.1],170.0)
