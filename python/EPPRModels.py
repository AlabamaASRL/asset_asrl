...
from SpiceRead import GetEphemTraj2,PoleVector
import Date as dt
from CR3BPModels import CR3BPFrame
from DataReadWrite import ReadData,WriteData,ReadCopernicusFile
...
class EPPRFrame(CR3BPFrame):
    ...
    def NDInertial_to_EPPR_Func(self):
        args = Args(16)
        X=args.head(3)
        V=args.segment3(3)
        t=args[6]
    ...
    def EPPR_to_NDInertial_Func(self):
        args = Args(16)
        Xrot=args.head(3)
    ...
    def Test(self,Name,N):
        ...
        epf = self.NDInertial_to_EPPR_Func()
        ipf = self.EPPR_to_NDInertial_Func()
    ...
    def AddSpiceBody(self,Name,mu,N = 5000):
    ...
    def GetPoleVector(self,Name,N=6000):
        ...
        return Poles
    def Add_P2_J2Effect(self,J2c,RadP2):
        self.P2_Rad = RadP2/self.lstar
        self.P2_J2  = J2c
        self.P2_PoleVectors = self.GetPoleVector("IAU_"+self.P2name,len(self.P2Data))
        self.P2_PoleFunc = oc.InterpFunction_3(oc.LGLInterpTable(3,self.P2_PoleVectors,len(self.P2Data))).vf()
        

            
    def EPPREOMs(self,r,v,t, otherGaccs=[], otherAccs=[], otherEOMs=[]):
         ...
         return vf.Stack([v,acc]+otherEOMs)
    ...
    def J2_ACC(self,r,t):
        ...
        if(self.P1_J2 != False):
             ...
             RP2 = r-self.P1
             Scale = 0.5*(self.mu)*self.P1_J1*(self.P1_Rad)**2
             dotterm = (vf.dot(RP2.normalized(),NP2))**2
    ...
    def Copernicus_to_EPPR(self,Filename,center="P2",SpiceFrame='J2000'):
        ...
        for T in data:
            X = np.zeros((7))
            tnd = self.JD_to_NDTime(T[7])
            X[0:6] = Tab.Interpolate(tnd)[0:6]
            X[0:3] += T[0:3]/self.lstar
            X[3:6] += T[3:6]/self.vstar
            X[6] = tnd
            Traj.append(X)
...
class EPPR(oc.ode_6.ode,CR3BPFrame):
    def __init__(self,Frame, Enable_J2 = False):
        self.Frame = Frame
        ...
        t =args[6]
        otherAccs = []
        if(Enable_J2==True):otherAccs+=Frame.J2_ACC(r,t)
        odeeq = self.Frame.EPPREOMs(r,v,t,otherAccs=otherAccs)
        oc.ode_6.ode.__init__(self,odeeq,6)
    
class EPPR_LT(oc.ode_6_3.ode,CR3BPFrame):
    def __init__(self,Frame,NonDim_LTacc = False, LTacc=False, Enable_J2 = False):
        self.Frame = Frame
        CR3BPFrame.__init__(self,self.Frame.P1mu,self.Frame.P2mu,self.Frame.lstar)
        ...
        elif(LTacc!=False):self.NDLTacc = LTacc/self.astar
        else:self.NDLTacc =NonDim_LTacc
        ...
        t =args[6]
        u = args.tail3()
        thrust = (u*self.NDLTacc)#*self.Frame.AccscaleFunc.eval(t)
        otherAccs = [thrust]
        if(Enable_J2==True):otherAccs+=Frame.J2_ACC(r,t)
        odeeq = self.Frame.EPPREOMs(r,v,t,otherAccs=otherAccs)
