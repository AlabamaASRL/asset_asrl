...
from SpiceRead import GetEphemTraj2,PoleVector,SpiceFrameTransform
import Date as dt
from DataReadWrite import ReadData,WriteData,ReadCopernicusFile
from TwoBodyFrame import TwoBodyFrame
from SpiceBodyProps import SpiceBodyProps as BProps
...
class NBodyFrame(TwoBodyFrame):
    ...
    def GetSpiceBodyTable(self,Name,N):
        T = self.GetSpiceBodyTraj(Name,N)
        return InterpTab(6,T,len(T))
    
    def AddTargetSpiceBody(self,Name,mu=None,brad = None, N = 5000):
        Table = self.GetSpiceBodyTable(Name,N)
        if(mu==None):mu=BProps[Name]["Mu"]
        if(brad==None):mu=BProps[Name]["Mu"]
