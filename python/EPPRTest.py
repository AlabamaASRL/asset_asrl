...
LTAcc = (0.35*(1.0e-3)*4.0*np.cos(20.0*c.dtr)/114.0)/3
LTAcc/= Frame.astar
args = Args(7)
...
Vs = np.linspace(-3.0,-0.5,10)
Trajs =[]
for V in Vs:
    IGC = np.copy(IG)
    t = IGC[6]
    IGC[3:6]+= normalize(IGC[3:6])*V*Frame.M_S(t)
    Trajs.append(integ2.integrate_dense(IGC,1.5))
...
sphase = sailode.phase(Tmodes.LGL5)
sphase.setTraj(SailIG,170)
